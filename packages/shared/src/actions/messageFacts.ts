...
export function cappedPlayersOnline(count: number | null | undefined, maxPlayers: number | null | undefined): number | null | undefined {
  return count === null || count === undefined ? count : Math.min(count, maxPlayers ?? Infinity)
}
...
export function messageFactValues(context: MessageFacts): Record<string, string> {
  ...
  return {
    ...
    players_online: number(cappedPlayersOnline(context.playersOnline, context.serverSlot)),
    serverSlot: number(context.serverSlot),
