...
test(`A2S enrichment is explicitly opt-in and validates booleans`, () => {
  assert.equal(parseGameServerListQuery({}).a2s, undefined)
  assert.equal(parseGameServerListQuery({ a2s: `true` }).a2s, true)
  assert.equal(parseGameServerListQuery({ a2s: false }).a2s, false)
  assert.throws(() => parseGameServerListQuery({ a2s: `yes` }), GameServerListQueryError)
})
