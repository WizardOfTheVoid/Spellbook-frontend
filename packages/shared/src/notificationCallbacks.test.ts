...
test(`accepts cancellation only for positive safe request IDs`, () => {
  assert.deepEqual(parseNotificationCallback({ label: `Cancel`, uri: `/requests/42/cancel` }), { label: `Cancel`, uri: `/requests/42/cancel` })
  for (const id of [`0`, `-1`, `1.5`, `9007199254740992`, `nope`]) {
    assert.equal(parseNotificationCallback({ label: `Cancel`, uri: `/requests/${id}/cancel` }), null)
  }
})
