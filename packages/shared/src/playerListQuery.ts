...
export const playerSortFields = [`rank`, `lastSeen`, `accountCreated`, `wantedAt`] as const
...
export function parsePlayerListQuery(value: unknown): ParsedPlayerListQuery {
	...
	const noRank = boolean(source.noRank, `noRank`)
	...
	if (noRank && (minRank !== undefined || maxRank !== undefined)) {
		throw new PlayerListQueryError(`noRank cannot be combined with rank bounds.`)
	}
