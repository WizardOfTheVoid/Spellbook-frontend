<script lang="ts">
	...
	import { sentinelModeEnabled } from "$lib/stores/sentinelModeStore"
	...
	$: void notificationInboxSession.sync(
		$authState.user?.onboardingComplete ? $authState.user.id : null,
	)
	$: syncGameProcessAvailability(
		$startupState.phase === "authenticated"
			&& Boolean($authState.user?.onboardingComplete)
			&& overlayVisible,
	)
	$: if ($startupState.phase === "authenticated" && $authState.user?.id !== warmedUserId) {
		cancelCacheWarmup()
		warmedUserId = $authState.user?.id ?? null
	...
	async function initializeOverlayVisibility(
		...
		try {
			...
			overlayVisible = await overlayApi.isVisible();
			setUnsubscribe(
				overlayApi.onVisibilityChange((visible) => {
					overlayVisible = visible;
				}),
			);
		} catch {
			overlayVisible = document.visibilityState === "visible";
		}
	}
