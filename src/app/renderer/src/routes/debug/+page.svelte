<script lang="ts">
	...
	import DebugRulesetPanel from '$lib/components/debug/debugRulesetPanel.svelte'
...
<div class="debug-hud debug-status" class:hud-hidden={!state?.enabled || state?.hudVisible === false} aria-label="Debug game panels">
	<DebugHudPanel name="tests" title="TESTS" status={state?.armed ? `NUMPAD ARMED` : `DISARMED`} layout={layout.tests}>
		...
		<p class="muted">Ctrl+F5 Debug · F6 HUD</p>
	...
	<DebugRulesetPanel data={state?.ruleset} layout={layout.ruleset} producerPaused={state?.pausedProducers.actions ?? false} />
