...
test(`missing PlayFab IDs require profile completion regardless of role or old onboarding state`, () => {
  assert.equal(needsPlayfabId(null), false)
  for (const isSuperadmin of [false, true]) {
    for (const playfabId of [null, ``, `   `, `ABC123`]) {
      assert.equal(needsPlayfabId({ ...session(isSuperadmin), playfabId, onboardingComplete: true }),
        playfabId !== `ABC123`)
    }
  }
})
