...
test(`keeps all tick actions visible without run history`, () => {
	...
	assert.deepEqual(actions.map(action => [action.action, action.status, action.supportsPause]), [
		...
		["servers", "idle", false],
		[`cleanup`, `idle`, false],
		[`normalizePlayerNames`, `idle`, false],
		[`enrichEmptyPlayers`, `idle`, false],
	])
	assert.equal(core.tickActionSummaries([]).find(action => action.action === `normalizePlayerNames`)?.label,
		`Norrmalize player names`)
})
