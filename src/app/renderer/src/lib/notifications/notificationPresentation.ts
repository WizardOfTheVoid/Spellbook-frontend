import { requestCancelId } from "./cancelNotificationRequest"
import {
	adminCustomNotificationSource,
	wantedExecutionNotificationSource,
	type NotificationRecord,
} from "@spellbook/shared/notifications.js"
import type { NotificationRequest } from "./notificationTypes.js"

export type NotificationPresentation = {
	title: string
	description: string | null
	icon: string | null
	iconType: `light` | `brands`
	avatarUrl: string | null
	avatarName: string | null
	callbackLabel: string | null
	read: boolean
	createdAt: string
}

export type NotificationArrivalPorts = {
	isCurrent(): boolean
	notify(request: NotificationRequest): void
	setRead(id: number, read: boolean): Promise<void>
	open(notification: NotificationRecord): Promise<void>
}

export function notificationPresentation(
	notification: NotificationRecord,
): NotificationPresentation {
	const sender = notification.source === adminCustomNotificationSource ? notification.meta : {}
	return {
		title: notification.title,
		description: notification.description,
		icon: notification.icon,
		iconType: notification.icon === `fa-discord` ? `brands` : `light`,
		avatarUrl: typeof sender.actorAvatarUrl === `string` && sender.actorAvatarUrl.trim() ? sender.actorAvatarUrl : null,
		avatarName: typeof sender.actorName === `string` ? sender.actorName : null,
		callbackLabel: notification.callback?.label ?? null,
		read: notification.readAt !== null,
		createdAt: notification.createdAt,
	}
}

export function notificationSfx(
	source: string,
): `notification` | `wanted-notification` {
	return source === wantedExecutionNotificationSource ?
		`wanted-notification`
	: 	`notification`
}

export function presentNotificationArrival(
	notification: NotificationRecord,
	ports: NotificationArrivalPorts,
): void {
	const presentation = notificationPresentation(notification)
	const wantedBan = notification.source === wantedExecutionNotificationSource && notification.content.actionType === `ban`
	if (!ports.isCurrent()) return
	ports.notify({
		message: presentation.title,
		description: presentation.description ?? undefined,
		variant: wantedBan ? `wanted` : undefined,
		gameServerId: typeof notification.content.gameServerId === `number` ? notification.content.gameServerId : undefined,
		createdAt: notification.createdAt,
		sound: wantedBan ? `badge` : notificationSfx(notification.source),
		level: notification.tone === `custom` ? `info` : notification.tone,
		icon: presentation.icon ?? undefined,
		iconType: presentation.iconType,
		avatarUrl: presentation.avatarUrl ?? undefined,
		avatarName: presentation.avatarName ?? undefined,
		dedupeKey: `durable-notification:${notification.id}`,
		action: notification.callback ? {
			label: notification.callback.label,
			...(requestCancelId(notification) !== null ? { icon: `fa-xmark` } : {}),
			onClick: async () => {
				if (!ports.isCurrent()) return
				await ports.setRead(notification.id, true)
				if (!ports.isCurrent()) return
				await ports.open(notification)
			},
		} : undefined,
	})
}
