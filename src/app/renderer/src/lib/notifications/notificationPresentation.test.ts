...
test(`custom notifications carry the sender avatar into the inbox and toast`, async () => {
	const { notificationPresentation, presentNotificationArrival } = await loadPresentation()
	assert.ok(notificationPresentation)
	assert.ok(presentNotificationArrival)
	const record = notification({
		source: `admin-custom-notification`,
		meta: { actorId: 9, actorName: `Admin`, actorAvatarUrl: `https://example.test/avatars/admin.webp` }
	})
	const presentation = notificationPresentation(record)
	assert.equal(presentation.avatarUrl, `https://example.test/avatars/admin.webp`)
	assert.equal(presentation.avatarName, `Admin`)
	const notices: NotificationRequest[] = []
	presentNotificationArrival(record, {
		isCurrent: () => true,
		notify: request => notices.push(request),
		setRead: async () => {},
		open: async () => {}
	})
	assert.equal(notices[0]?.avatarUrl, `https://example.test/avatars/admin.webp`)
	assert.equal(notices[0]?.avatarName, `Admin`)
	assert.equal(notificationPresentation({ ...record, source: `admin-notification-test` }).avatarUrl, null)
	assert.equal(notificationPresentation({ ...record, meta: { actorAvatarUrl: null } }).avatarUrl, null)
})
...
test(`emits one custom-tone toast and sound whose CTA persists read state before navigation`, async () => {
	...
	assert.equal(notices.filter(notice => notice.sound).length, 1)
	assert.deepEqual(notices.map(notice => notice.sound), [`notification`])
...
test(`a re-entrant reset can discard the arrival and its sound together`, async () => {
	const presentArrival = (await loadPresentation()).presentNotificationArrival
	assert.ok(presentArrival)
	const notices: NotificationRequest[] = []
	let current = true
...
test(`a Wanted ban carries its description, server scope and badge cue through notifications`, async () => {
	const { presentNotificationArrival } = await loadPresentation()
	assert.ok(presentNotificationArrival)
	const notices: NotificationRequest[] = []
	const record = notification({ source: wantedExecutionNotificationSource, content: { actionType: `ban`, gameServerId: 8 } })
	presentNotificationArrival(record, {
		isCurrent: () => true,
		notify: request => notices.push(request),
		setRead: async () => {},
	...
	assert.equal(notices[0]?.description, record.description)
	assert.equal(notices[0]?.gameServerId, 8)
	assert.equal(notices[0]?.createdAt, record.createdAt)
	assert.equal(notices[0]?.variant, `wanted`)
	assert.equal(notices[0]?.sound, `badge`)
})
