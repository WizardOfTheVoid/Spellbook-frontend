	...
	return {
		request(teamId: number): Promise<void> {
			...
			return new Promise<void>((resolve, reject) => {
				current = { requestId, teamId, resolve, reject }
				onChange({ requestId, teamId })
			})
		},
		handled(requestId: number, error?: unknown): void {
...
export function resolveNotificationCallback(uri: string): NotificationNavigationTarget | null {
	...
	if (page === 'teams' && id) return { page, teamId: Number(id) }
...
export async function openNotificationTarget(
	...
	try {
		...
		if ('teamId' in target) await ports.selectTeam(target.teamId)
