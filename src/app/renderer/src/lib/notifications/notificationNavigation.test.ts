...
test('resolves supported top-level, team, and Wanted callback targets', async () => {
	...
	assert.deepEqual(resolveNotificationCallback('/teams/7/requests'), { page: 'teams', teamId: 7, teamView: 'requests' })
...
test(`opens join notifications directly on the requested team's requests`, async () => {
	const { openNotificationTarget } = await loadNavigation()
	const calls: unknown[] = []
	assert.equal(await openNotificationTarget(`/teams/7/requests`, navigationPorts({
		selectTeam: async (teamId, teamView) => { calls.push({ teamId, teamView }) }
	})), true)
	assert.deepEqual(calls, [{ teamId: 7, teamView: `requests` }])
})
