<script lang="ts">
	...
	let rangeValue = 35;
	const rangeSteps = [1, 24, 192, 1024]
	let customRangeValue = 24
	let customMinimum = 24
	let customMaximum = 192
	let freeRangeValue = 101
	let freeMinimum = 101
	let freeMaximum = 501
	let minimumRank = 120;
...
<section {hidden} class="panel-view gallery" aria-label="UI primitives">
	...
	<section>
		<h2>DateStamp</h2>
		<div class="specimens">
			<span class="specimen"><DateStamp value="2026-09-25T14:32:00.000Z" /><code>date</code></span>
			<span class="specimen"><DateStamp value="2026-09-25T14:32:00.000Z" format="dateTime" /><code>date and time</code></span>
			<span class="specimen"><DateStamp value="2026-09-25T14:32:00.000Z" format="time" /><code>time</code></span>
			<span class="specimen"><DateStamp value="2026-09-25T14:32:00.000Z" format="relative" /><code>relative</code></span>
			<span class="specimen"><DateStamp value="2026-09-25T14:32:00.000Z" format="dateTime" styled={false} /><code>inherit style</code></span>
		</div>
	</section>
	...
	<section>
		...
		<p>Click a dot to edit its number, or click a step to snap to it.</p>
		<div class="control-grid">
			<Range
			...
			<Range
				...
				onChange={(next) => customRangeValue = next}
			/>
			<DoubleRange
				label="Custom step interval"
				steps={rangeSteps}
				minimumValue={customMinimum}
				maximumValue={customMaximum}
				onChange={(minimum, maximum) => {
			...
			/>
			<Range
				label="Allow out-of-step values (step 100)"
				max={1000}
				step={100}
				allow-out-of-step={true}
				value={freeRangeValue}
				onChange={(next) => freeRangeValue = next}
			/>
			<DoubleRange
				label="Allow out-of-step interval (step 100)"
				max={1000}
				step={100}
				allow-out-of-step={true}
				minimumValue={freeMinimum}
				maximumValue={freeMaximum}
				onChange={(minimum, maximum) => {
					freeMinimum = minimum
					freeMaximum = maximum
				}}
