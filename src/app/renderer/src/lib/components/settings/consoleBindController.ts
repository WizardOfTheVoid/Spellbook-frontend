...
type ConsoleBindDependencies<Key extends string> = {
  load: () => Promise<boolean>
  save: (consoleKey: Key | null) => Promise<boolean>
}
...
export class ConsoleBindController<Key extends string = ConsoleKeyCode> {
  state = initialState

  private active = true
  private pendingKey: Key | null | undefined
  private revision = 0

  constructor(
    private readonly dependencies: ConsoleBindDependencies<Key>,
    private readonly onChange: (state: ConsoleBindState) => void = () => undefined,
    private readonly label = `Console key`
  ) {}
  ...
  async load(): Promise<void> {
    ...
    this.setState({
      ...initialState,
      message: loaded ? `` : `Could not load ${this.label}. Try again.`,
  ...
  rejectKey(message = `Use one key without modifiers. Enter, Escape and DevTools are reserved.`): void {
    if (!this.active || !this.state.recording) return
    this.setState({
      ...this.state,
      message
  ...
  async save(consoleKey: Key | null): Promise<void> {
    if (!this.active || this.state.loading || this.state.saving) return
    const request = ++this.revision
    ...
    this.setState({
      ...initialState,
      message: saved ? `${this.label} saved.` : `Could not save ${this.label}. Try again.`,
