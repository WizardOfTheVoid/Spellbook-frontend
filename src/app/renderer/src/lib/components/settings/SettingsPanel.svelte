<script lang="ts">
	...
	onMount(() => window.chivDebug?.onState(state => {
		settingsSnapshot.update(current => current && current.settings.debug !== state.enabled
			? { ...current, settings: { ...current.settings, debug: state.enabled } }
			: current)
	}))
...
<section {hidden} class="panel-view settings-view" aria-label="Settings">
	...
	<div class="settings-list" use:navigationScroll={`settings-list`}>
		<section class="settings-section" aria-labelledby="settings-console">
			...
			<KeybindSetting name="consoleKey" {hidden} disabled={saving} bind:busy={consoleKeyBusy} />
		...
		<section class="settings-section" aria-labelledby="settings-shortcuts">
			...
			<KeybindSetting name="quickOpenPlayerKey" {hidden} disabled={saving} />
		</section>
		<hr />
		...
		<section class="settings-section" aria-labelledby="settings-diagnostics">
			...
			<div class="settings-row">
				...
				<span class="settings-row__copy"><strong>Diagnostic logs</strong><small>Save a log file to attach to a Discord bug report.</small></span>
				<LogSharingButton />
			</div>
			<div class="settings-row">
				<IconBadge name="fa-bug" tone="accent" />
				<span class="settings-row__copy"><strong>Debug</strong><small>Test Actions and inspect game activity, the queue, and input.</small></span>
				<div class="debug-actions">
					<Button label="Open controls" onClick={() => debugControlsOpen = true} />
					<Toggle label="Debug" showLabel={false} checked={settings?.debug === true}
						disabled={!settings || saving} onChange={(debug) => void save({ debug })} />
				</div>
			</div>
		</section>
	</div>
...
<style lang="scss">
	.debug-actions { display: flex; align-items: center; gap: var(--gutter-md); }
	.settings-view {
		height: 100%;
		display: grid;
	...
	.settings-section { display: grid; gap: var(--gutter-md); min-width: 0; }
	h2 { margin: 0; font-size: var(--font-size-md); color: var(--color-light-secondary); }
	hr { width: 100%; margin: var(--gutter-md) 0; border: 0; border-top: 1px solid var(--color-dark-tertiary); }
