...
<div class="discord-team-tile" bind:this={root}>
	...
	/>
	{#if state.status === `connected`}
		<p class="discord-setup">Create a channel in your Discord server, then run <code>/sb-config</code> in that channel to finish setting up SpellBook.</p>
	{/if}
</div>
...
<style lang="scss">
	...
	.discord-setup { margin: 0; color: var(--color-light-secondary); font-size: var(--font-size-xs); line-height: 1.5; }
</style>
