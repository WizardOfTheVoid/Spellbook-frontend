<script lang="ts">
	...
	async function decide(userId: number, decision: `approve` | `reject`): Promise<void> {
		...
		try {
			...
			void pendingTeamRequests.refresh()
...
{#if view === `requests`}
	...
	{/if}
{:else}
	<Tile title="Join requests" icon="fa-user-clock" subtitle={error ? `Could not load requests. Open to retry.` : `Review new members`} badge={requests.length} onClick={() => void navigation.visit(async () => { view = `requests`
		await load() })} />
{/if}
