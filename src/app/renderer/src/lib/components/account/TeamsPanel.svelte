<script lang="ts">
	...
	rememberNavigation(`teamDetail`, () => ({ selectedTeamId, teamView }), async (state, isCurrent) => {
		if (!state.selectedTeamId) { closeTeam(); return }
		if (!loaded) await loadTeams()
		if (!isCurrent()) return
		if (!teams.some(team => team.id === state.selectedTeamId)) { closeTeam(); return }
		await selectTeam(state.selectedTeamId)
		if (isCurrent() && selectedTeamId === state.selectedTeamId) {
			teamView = state.teamView === `requests` && !User.Ability.can(`admin`, { type: `team`, id: selectedTeamId }) ? `members` : state.teamView
		}
	})
</script>
...
<section {hidden} class="panel-view teams-panel" aria-label="My teams">
	...
	{#if selectedTeam}
		<div class="teams-detail panel-subview grid-stack gap-1" use:navigationScroll={`teams-detail`}>
	...
	{:else}
		<div class="teams-list panel-subview gap-1" use:navigationScroll={`teams-list`}>
