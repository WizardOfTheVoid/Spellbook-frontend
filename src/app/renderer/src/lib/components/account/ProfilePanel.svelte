<script lang="ts">
	...
	import ConsoleBindSetting from "$lib/components/settings/ConsoleBindSetting.svelte"
	...
	async function save(): Promise<void> {
		if (saving || consoleBindBusy || !displayName.trim() || !playfabId.trim()) return;
		saving = true;
		try {
...
<section {hidden} class="panel-view account-panel" aria-label="Profile">
	...
	<div class="profile-content panel-subview grid-stack gap-125" use:navigationScroll={`profile-content`}>
		...
		<form class="profile-form" on:submit|preventDefault={() => void save()}>
			...
			/>
			<ConsoleBindSetting {hidden} disabled={saving} bind:busy={consoleBindBusy} />
			<Input
			...
			<div class="profile-form__actions">
				<Button
					...
					icon="fa-floppy-disk"
					variant="primary"
					disabled={saving || consoleBindBusy || !displayName.trim() || !playfabId.trim()}
					onClick={() => void save()}
				/>
			</div>
		</form>
