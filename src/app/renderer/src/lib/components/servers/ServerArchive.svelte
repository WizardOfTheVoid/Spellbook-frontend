<script lang="ts">
	import { rememberNavigation, navigationScroll } from "$lib/navigation/navigation"
	...
	rememberNavigation(`serverArchive`, () => ({ page, searchInput, search, filters, queryFilters, advancedFiltersOpen }), value => {
		({ page, searchInput, search, filters, queryFilters, advancedFiltersOpen } = value)
		debouncer.cancel()
	}, () => null)
</script>
...
<div class="server-archive" {hidden}>
	...
	<div class="server-archive__body" use:navigationScroll={`serverArchive`} aria-busy={loading}>
