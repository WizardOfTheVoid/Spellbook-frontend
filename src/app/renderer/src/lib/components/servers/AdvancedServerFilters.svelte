<script lang="ts">
	...
	$: gameModeOptions = [
		{ value: ``, label: `Any game mode` },
		...gameModes.map((value) => ({ value, label: value })),
	];
</script>
...
<section class="advanced-server-filters" id="advanced-server-filters">
	...
	<div class="advanced-server-filters__grid">
		...
		<div class="advanced-server-filters__grid advanced-server-filters__wide">
			<DoubleRange
				label="Max slots"
				min={1}
				max={90}
				minimumValue={filters.minSlots}
				maximumValue={filters.maxSlots}
				onChange={(minSlots, maxSlots) => update({ minSlots, maxSlots })}
			/>
			...
			<DoubleRange
				...
				min={0}
				max={90}
				minimumValue={filters.minPlayers}
				maximumValue={filters.maxPlayers}
				onChange={(minPlayers, maxPlayers) =>
					update({ minPlayers, maxPlayers })}
			/>
		</div>
		...
		<div class="advanced-server-filters__grid advanced-server-filters__wide">
			<Select
				...
				options={sortOptions}
				onChange={(sortBy) =>
					update({ sortBy: sortBy as ServerFilterState["sortBy"] })}
			/>
			{#if filters.sortBy !== `default`}<Select
					label="Order"
					value={filters.sortOrder}
					options={orderOptions}
					onChange={(sortOrder) =>
						update({ sortOrder: sortOrder as ServerFilterState["sortOrder"] })}
				/>{/if}
			{#if isSuperadmin}
				<div class="advanced-server-filters__wide">
					<Checkbox
						label="Deleted"
						description="Show only soft-deleted servers."
						checked={filters.deleted === `deleted`}
						onChange={(checked) =>
							update({ deleted: checked ? `deleted` : `active` })}
					/>
				</div>
			{/if}
		</div>
...
<style lang="scss">
	...
	.advanced-server-filters__grid {
		display: grid;
		grid-template-columns: repeat(2, minmax(0, 1fr));
		gap: var(--gutter-md) var(--gutter-lg);
	}
	.advanced-server-filters__wide {
		grid-column: 1 / -1;
	}
</style>
