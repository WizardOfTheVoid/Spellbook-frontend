<script lang="ts">
	...
	$: if (view === `players` && !profile?.canViewPlayers) view = `details`
...
<div class="server-editor panel-subview grid-stack gap-125" class:server-editor--players={view === `players`}>
	<nav class="server-editor__tabs" aria-label="Server editor sections">
		...
		{#if profile?.canViewPlayers}
			<button type="button" aria-pressed={view === `players`} on:click={() => (view = `players`)}>Players</button>
		{/if}
