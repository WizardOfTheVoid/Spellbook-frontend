...
<div class="server-editor panel-subview grid-stack gap-125" class:server-editor--players={view === `players`} use:navigationScroll={`serverEditor`}>
	<nav class="server-editor__tabs" aria-label="Server editor sections">
		{#if profile?.ownerTeamId}
			<Button label="Team" icon="fa-users" onClick={() => onOpenTeam(profile!.ownerTeamId!)} />
		{/if}
		<button type="button" aria-pressed={view === `details`} on:click={() => void navigation.visit(() => { view = `details` })}>Details</button>
		{#if profile?.canViewPlayers}
			<button type="button" aria-pressed={view === `players`} on:click={() => void navigation.visit(() => { view = `players` })}>Players</button>
		{/if}
		...
		>
			<button
				...
				on:click={() => void navigation.visit(openVariables)}
			>Variables</button>
		</span>
	</nav>
