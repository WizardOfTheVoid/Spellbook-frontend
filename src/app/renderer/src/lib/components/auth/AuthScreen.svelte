...
<main class="auth-screen">
	<section class="auth-panel">
		...
		{:else if !user}
			...
			{#if accountNotice}
				{#if startupErrorCode === "ACCOUNT_SUSPENDED"}
					...
					<p class="account-notice">If you think this was a mistake, create a ticket on Discord.</p>
				{:else}
					<p class="account-notice">{accountNotice.message}</p>
				{/if}
				<p class="account-help">
		...
		{#if error && !accountNotice}<p class="auth-error" role="alert">
				{error}
			</p>{/if}
	</section>
</main>
