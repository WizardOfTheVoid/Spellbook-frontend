<script lang="ts">
	...
	import { observeNotificationRead } from "$lib/notifications/notificationReadObserver"
	import { clickedRequestCancels, requestCancelId } from "$lib/notifications/cancelNotificationRequest"
	...
	function observeMore(node: HTMLElement): { destroy(): void } {
		const observer = new IntersectionObserver(entries => {
			if (!state.error && entries.some(entry => entry.isIntersecting)) void loadMore()
		}, { root: node.closest(`.notifications-view__body`), rootMargin: `100px 0px` })
	...
	async function markUnread(notification: NotificationRecord): Promise<void> {
		await mutate(notification.id, async () => {
			manuallyUnreadIds = new Set(manuallyUnreadIds).add(notification.id)
			try {
				await onSetRead(notification.id, false)
			} catch (error) {
				const next = new Set(manuallyUnreadIds)
				next.delete(notification.id)
				manuallyUnreadIds = next
				throw error
			}
		})
	}
...
<section class="panel-view notifications-view" aria-label="Notifications">
	<PanelHeader title="Notifications" eyebrow={`Inbox (${state.unreadCount}/${state.totalCount})`}>
		<svelte:fragment slot="trailing">
			...
			<IconButton
				...
				tone="danger"
				disabled={deletingAll || state.totalCount === 0}
				onClick={() => {
					deleteError = null
					showDeleteAll = true
				}}
	...
	<div class="notifications-view__body" use:navigationScroll={`notifications-view__body`}>
		{#if state.error}
		...
		{#if notifications.length}
			<div class="notification-inbox" aria-live="polite">
				{#each notifications as notification (notification.id)}
					...
					{@const cancelId = requestCancelId(notification)}
					<article
						use:observeNotificationRead={{ active: isActive && !manuallyUnreadIds.has(notification.id), read: presentation.read, markRead: () => onSetRead(notification.id, true) }}
						class={`notification-row notification-row--${notification.tone}`}
						class:notification-row--unread={!presentation.read}
					>
						<span class="notification-row__icon" aria-hidden={presentation.avatarUrl ? undefined : true}>
							<NotificationIcon
							...
							{#if presentation.callbackLabel}
								<span class="notification-row__action">
									<Button
										...
										icon={cancelId === null ? `fa-arrow-up-right-from-square` : `fa-xmark`}
										size="sm"
										disabled={pendingIds.has(notification.id) || (cancelId !== null && $clickedRequestCancels.has(cancelId))}
						...
						<span class="notification-row__controls">
							{#if presentation.read}
								<IconButton
									icon="fa-envelope"
									ariaLabel="Mark notification as unread"
		...
		{#if state.loadingMore}
			<p class="notifications-view__more" role="status">Loading more notifications...</p>
		{:else if state.hasMore && !state.loading}
			<div class="notifications-view__more" use:observeMore>
				<Button label={state.error ? `Retry loading` : `Load more`} size="sm" onClick={() => void loadMore()} />
			</div>
		{/if}
	</div>
</section>
...
{#if showDeleteAll}
	<ConfirmModal
		...
		message="This deletes every notification in your inbox, including older notifications that haven't loaded yet."
	...
	>
		{#if deleteError}<p class="notifications-view__error" role="alert">{deleteError}</p>{/if}
	</ConfirmModal>
{/if}
