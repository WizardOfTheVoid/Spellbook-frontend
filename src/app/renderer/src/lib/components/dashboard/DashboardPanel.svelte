<script lang="ts">
	...
	rememberNavigation(`dashboardTabs`, () => ({ selectedAdminTab, selectedActionTab }), state => { ({ selectedAdminTab, selectedActionTab } = state) })
</script>
...
<section class="panel-view dashboard" aria-label="Dashboard">
	...
	{#if data}
		<div class="dashboard__body" use:navigationScroll={`dashboard`}>
			...
			<div class="dashboard__stats">
				<DashboardStatCard label="Global local bans" value={data.global.localBans.toLocaleString()} caption="recorded human actions" />
				<DashboardStatCard label="Global Wanted actions" value={data.global.wantedActions.toLocaleString()} caption={`${data.global.wantedServerApplications.toLocaleString()} automatic applications`} />
				<DashboardStatCard label="Global player actions" value={data.global.playerActions.toLocaleString()} caption="all communities" />
				<DashboardStatCard label="Admins online" value={data.global.activeAdmins} caption="active in the last 15 minutes" />
			</div>
			...
			<div class="dashboard__lower-grid">
				<TabbedListCard title="Top admins" caption="Global · last 30 days" tabs={adminTabs} selected={selectedAdminTab} onSelect={value => void navigation.visit(() => { selectedAdminTab = value })}>
				...
				<TabbedListCard title="Recent player actions" caption="Latest visible actions" tabs={actionTabs} selected={selectedActionTab} onSelect={value => void navigation.visit(() => { selectedActionTab = value })}>
...
<style lang="scss">
	...
	.dashboard-card { min-width: 0; border: 1px solid var(--color-dark-secondary); border-radius: var(--radius); background: rgbaa(var(--color-dark-primary), 0.34); }
	...
	.dashboard-card small { color: var(--color-light-tertiary); font-size: var(--font-size-xs); }
