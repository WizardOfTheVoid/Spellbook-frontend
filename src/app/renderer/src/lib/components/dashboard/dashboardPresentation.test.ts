...
test(`uses the live source contract`, async () => {
  const panel = await readFile(new URL(`./DashboardPanel.svelte`, import.meta.url), `utf8`)

  assert.doesNotMatch(panel, /getDashboardPreviewData/u)
  assert.doesNotMatch(panel, /Preview/u)
  assert.doesNotMatch(panel, /WIP/u)
})
