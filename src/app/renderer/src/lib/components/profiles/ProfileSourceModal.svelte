...
>
	<Select label="Start from" options={sourceOptions} value={source} disabled={submitting} onChange={selectSource} />
	{#if source === `team`}
		<Select label="Team" options={teamOptions} value={teamKey} placeholder="Select team" disabled={submitting} onChange={value => void selectTeam(value)} />
		{#if teamKey}
			<Select label="Profile" options={profileOptions} value={profileKey} placeholder={loading ? `Loading profiles…` : `Select profile`} disabled={loading || submitting} onChange={value => profileKey = value} />
		{/if}
