<script lang="ts">
	...
	async function loadOwnerOptions(): Promise<void> {
		try {
			ownerOptions = await fetchProfileOwners();
	...
	function selectOwner(value: string): void {
	...
	async function openProfile(profileId: number): Promise<void> {
		...
		try {
			profile = await fetchServerProfile(selectedOwner, profileId);
	...
	async function cancelProfile(): Promise<void> {
		...
		await openProfile(profile.profile.id);
	}
	...
	function setCommandType(command: ServerProfileCommand, value: string): void {
		...
		if (command.commandType === "server_message") {
	...
	function defaultCommand(
		...
		return {
			...
			offenseType: type === "server_message" ? null : "other",
		};
	}
		...
		{:else if view === "command" && selectedAction && selectedCommand && selectedCommandIndex !== null}
			<div class="profile-screen grid-stack gap-125">
				<section class="profile-card grid-stack gap-125">
					...
					{/if}
					{#if selectedCommand.commandType !== "server_message"}
						<Select
							label="Offense type"
