<script lang="ts">
	...
	$: if (
		...
		void openProfile(selectedProfileId, Boolean(navigation.restored(`profileView`)));
	}
	...
	function openSummaryMenu(
		...
		openProfileMenu(event, {
			...
			onOpen: async () => { await navigation.visit(() => openProfile(summary.profile.id)) },
			onDuplicate: async () => { await navigation.visit(() => duplicateProfile(summary)) },
	...
	function setCommandType(command: ServerProfileCommand, value: string): void {
		...
		if (
			command.commandType === "server_message" ||
	...
	function defaultCommand(
		...
		return {
			...
			offenseType:
				type === "server_message" || type === `unban` ? null : "other",
		};
	}
	...
	type ProfileNavigation = { view: ProfileView, mode: ProfileMode, draft: ServerProfileGraph | null,
		actionIndex: number | null, commandIndex: number | null, profileId: number | null, actionId: number | null, commandId: number | null }
	function restoreProfileNavigation(state: ProfileNavigation) {
		...
		view = state.view
		selectedActionIndex = state.actionId ? profile?.actions.findIndex(action => action.id === state.actionId) ?? -1 : state.actionIndex
		if (selectedActionIndex !== null && (!profile || !profile.actions[selectedActionIndex])) {
			selectedActionIndex = null
			view = profile ? `profile` : `overview`
		}
		const action = selectedActionIndex === null ? null : profile?.actions[selectedActionIndex]
		selectedCommandIndex = state.commandId ? action?.commands.findIndex(command => command.id === state.commandId) ?? -1 : state.commandIndex
		if (selectedCommandIndex !== null && !action?.commands[selectedCommandIndex]) {
			selectedCommandIndex = null
			if (view === `command`) view = action ? `action` : `profile`
		}
	}
	rememberNavigation(`profileView`, (): ProfileNavigation => ({ view, mode, draft: mode === `creating` ? initialDraft : null,
		actionIndex: selectedActionIndex, commandIndex: selectedCommandIndex,
		profileId: profile?.profile.id ?? selectedProfileId, actionId: selectedAction?.id ?? null, commandId: selectedCommand?.id ?? null }), state => {
...
>
	...
	<div
		class="profiles-view__body" use:navigationScroll={`profiles-view__body`}
				...
				{#each summaries as summary (summary.profile.id)}
					...
					>
						<Tile
							...
							onClick={() => void navigation.visit(() => openProfile(summary.profile.id))}
							icon="fa-layer-group"
							subtitle={`${summary.serverCount} servers / ${summary.enabledActionCount}/${summary.actionCount} actions / ${summary.commandCount} commands`}
							suffix={[
			...
			<div class="profile-screen grid-stack gap-125">
				<section class="profile-card grid-stack gap-125">
					...
					{:else if profile.profile.isPreset}
						<small
							>Only superadmins can edit this preset. Duplicate it to make your
							own changes.</small
						>
					{/if}
				...
				<section class="profile-card grid-stack gap-125">
					<div class="profile-card-header">
						<h2>
							Servers
							<HelpHint
								text="Choose which servers your profile applies to. These servers are marked as owned by your team."
							/>
						</h2>
						<small>{profile.servers.length} servers claimed</small>
					</div>
					{#if profile.profile.owner.type === `team`}
						<small
							>Only one team can claim a server. Is your server claimed by
							someone else? <a
								href="https://chivalry2.dev/discord"
								on:click|preventDefault={() => void window.chivAuth.openHelp()}
								>Create a discord ticket</a
							></small
						>
					{/if}
					{#if gameServers.length > 0}
						...
						/>
					{:else}
						<small>No game servers exist in SpellBook.</small>
					{/if}
				</section>
				...
				<section class="profile-card grid-stack gap-125">
					...
					{#if selectedCommand.commandType === "ban"}
						...
						<Button
							label="MAX"
							size="sm"
							onClick={() => setMaxDuration(selectedCommand)}
						/>
					{/if}
...
{#if sourceModal}
	<ProfileSourceModal
		...
		owners={ownerOptions}
		onSelect={source => void navigation.visit(() => applyProfileSource(source))}
