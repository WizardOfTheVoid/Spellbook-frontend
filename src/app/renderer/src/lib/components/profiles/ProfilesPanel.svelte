<script lang="ts">
	...
	const commandAppearance: Record<ServerProfileCommandType, { icon: string, tone: string }> = {
	...
	const unselectedCommandAppearance = { icon: `fa-circle-question`, tone: `var(--color-light-tertiary)` }
	...
	function setCommandType(command: ServerProfileCommand, value: string): void {
		...
		command.incrementalBan =
			...
			:	null;
		command.durationHours = value === `ban` ? command.durationHours ?? 24 : null
		if (
	...
	function defaultCommand(
		...
		return {
			...
			sortOrder,
			delayMs: 15,
			durationHours: type === `ban` ? 24 : null,
			message:
			...
			<div class="profile-screen grid-stack gap-125">
				...
				<section class="profile-card grid-stack gap-125">
					...
					{#each selectedAction.commands as command, commandIndex (command)}
						{@const summary = commandRowSummary(command)}
						{@const appearance = commandAppearance[command.commandType] ?? unselectedCommandAppearance}
						<button
							class="profile-row-button profile-row-button--command"
						...
						>
							<span class="profile-command-icon">
								<Icon name={appearance.icon} tone={appearance.tone} />
							</span>
							<strong>{labelCommandType(command.commandType)}</strong>
							<small class:profile-message-command={isMessageCommand(command.commandType)}>{summary.text}{#if summary.message} / <span class="profile-command-message">{summary.message}</span>{/if}</small>
						</button>
					{/each}
				</section>
			</div>
...
<style lang="scss">
	...
	.profile-row-button--command {
		--color-command-kick: rgb(255 170 102);
		grid-template-columns: auto minmax(0, 1fr);
	}

	.profile-action-icon,
	.profile-command-icon {
		grid-row: span 2;
		display: grid;
		place-items: center;
	}
	...
	.profile-row-button small.profile-message-command {
		color: var(--color-light-secondary);
