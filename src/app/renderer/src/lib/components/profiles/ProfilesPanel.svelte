<script lang="ts">
	...
	function profileState(): string {
		return profile ? JSON.stringify([profileInput(profile, true), transferOwnerKey]) : ``
	}
	...
	$: canEditProfile = canManageProfile(profile?.profile.owner ?? null, mode === `creating` ? `create` : `edit`, $authState.user, ownerOptions, profile?.profile.isPreset)
	$: canDeleteProfile = Boolean(profile && !profile.profile.isDefault && canManageProfile(profile.profile.owner, `delete`, $authState.user, ownerOptions, profile.profile.isPreset))
	$: editorDisabled = saving || !canEditProfile
	$: editingDefault = view !== `overview` && profile?.profile.isDefault && canEditProfile
	...
	async function duplicateProfile(summary: ServerProfileSummary): Promise<void> {
		...
		try {
			...
			if (revision !== profileRevision || selectedOwnerKey !== ownerKey || !profilePreferencesState.isCurrent(session)) return
			savedProfile = null
			savedProfileState = ``
			profile = newProfileDraft(destination, source, summaries.map(item => item.profile.name))
			loadedProfileId = null
	...
	async function setProfileEnabled(summary: ServerProfileSummary, isEnabled: boolean): Promise<void> {
		if (!selectedOwner || summary.profile.isDefault || preferenceSavingIds.has(summary.profile.id)) return
		...
		try {
			...
			if (profilePreferencesState.isCurrent(session)) notifyError(getError(saveError, `Profile preference save failed.`))
		} finally {
			if (profilePreferencesState.isCurrent(session)) setPreferenceSaving(summary.profile.id, false)
		}
	}

	function openSummaryMenu(event: MouseEvent, summary: ServerProfileSummary): void {
		openProfileMenu(event, {
			summary,
			canDuplicate: canCreateProfile,
			busy: loading || saving || preferenceSavingIds.has(summary.profile.id),
			onOpen: () => openProfile(summary.profile.id),
			onDuplicate: () => duplicateProfile(summary),
			onSetEnabled: isEnabled => setProfileEnabled(summary, isEnabled),
		})
	}

	function openSummaryFromRow(event: MouseEvent, summary: ServerProfileSummary): void {
		const target = event.target
		if (target instanceof Element && target.closest(`button, a, input, select, textarea`)) return
		void openProfile(summary.profile.id)
	}

	function openSummaryFromKeyboard(event: KeyboardEvent, summary: ServerProfileSummary): void {
	...
	async function saveProfile(): Promise<void> {
		...
		try {
			...
			const input = mode === `creating` || !savedProfile ? profileInput(profile, canPublish) : profileChanges(profile, savedProfile, canPublish)
	...
	function setDuration(command: ServerProfileCommand, value: number): void {
		if (editorDisabled) return
		command.durationHours = Math.max(1, Math.min(maxBanHours, value))
		touchProfile();
	}

	function setMaxDuration(command: ServerProfileCommand): void {
		if (editorDisabled) return
		command.durationHours = maxBanHours
		touchProfile();
	}
...
<section {hidden} class="panel-view profiles-view" class:profiles-view--default={editingDefault} aria-label="Profiles">
	<PanelHeader
		title={panelTitle}
	...
	{#if editingDefault}
		<aside class="profile-default-notice" role="note">
			<Icon name="fa-triangle-exclamation" tone="var(--color-accent-tertiary)" />
			<p><strong>You're editing the default profile.</strong> Saved changes affect everyone using Default.</p>
		</aside>
	{/if}
			...
			<div class="profile-screen grid-stack gap-125">
				...
				{#each summaries as summary (summary.profile.id)}
					<div
						...
						class:profile-summary--selected={$infinityMenuState?.owner?.dataset.profileSummaryId === `${summary.profile.id}`}
					...
					>
						<span class="profile-summary__copy">
							...
							{#if summary.profile.isPreset}<span>Preset</span>{/if}
							{#if summary.isEnabledForUser === false}<span>Disabled for you</span>{/if}
						</span>
						<IconButton
							icon="fa-ellipsis-vertical"
							ariaLabel={`Actions for ${summary.profile.name}`}
							size="sm"
							stopPropagation
							expanded={$infinityMenuState?.owner?.dataset.profileSummaryId === `${summary.profile.id}`}
							onClick={(event) => openSummaryMenu(event, summary)}
						/>
					</div>
				{:else}
					<EmptyState
		...
		{:else if view === "profile" && profile}
			<div class="profile-screen grid-stack gap-125">
				<section class="profile-card grid-stack gap-125">
					...
					/>
					{#if $authState.user?.isSuperadmin && !profile.profile.isDefault}
						<Toggle label="Preset" description={profile.profile.isPreset ? `Yes` : `No`}
							tooltip="Make this profile available for all admins to copy. Changes apply when you save."
							checked={profile.profile.isPreset === true} disabled={editorDisabled}
							onChange={value => { if (profile) profile.profile.isPreset = value }} />
					{:else if profile.profile.isPreset}
						<small>Only superadmins can edit this preset. Duplicate it to make your own changes.</small>
					{/if}
				...
				<section class="profile-card grid-stack gap-125">
					<div class="profile-card-header">
						<h2>Servers</h2>
						<small>{profile.servers.length} claimed</small>
					</div>
				...
				<fieldset class="profile-card grid-stack gap-125" disabled={editorDisabled}>
					<div class="profile-card-header">
						...
						<div class="grid-cluster gap-05">
							<Button label="Up" sfx="reorder" size="sm" disabled={selectedActionIndex === 0} onClick={() => moveSelectedAction(-1)} />
							<Button label="Down" sfx="reorder" size="sm" disabled={selectedActionIndex === profile.actions.length - 1} onClick={() => moveSelectedAction(1)} />
			...
			<fieldset class="profile-screen grid-stack gap-125" disabled={editorDisabled}>
				<section class="profile-card grid-stack gap-125">
					<div class="profile-card-header">
						<h2>Command</h2>
						<div class="grid-cluster gap-05">
							<Button label="Up" sfx="reorder" size="sm" disabled={selectedCommandIndex === 0} onClick={() => moveSelectedCommand(-1)} />
							<Button label="Down" sfx="reorder" size="sm" disabled={selectedCommandIndex === selectedAction.commands.length - 1} onClick={() => moveSelectedCommand(1)} />
					...
					/>
					{#if selectedCommand.commandType === "ban"}
						<Range
							label="Time hours"
							min={1}
							max={maxBanHours}
							value={Math.min(selectedCommand.durationHours ?? 24, maxBanHours)}
							formatValue={() => `${selectedCommand.durationHours ?? 24}`}
...
<style lang="scss">
	...
	.profile-default-notice p { margin: 0; }
	.profile-default-notice strong { color: var(--color-accent-tertiary); }
