import type { ProfileOwner, ProfileOwnerOption, ServerProfileAction, ServerProfileCommand, ServerProfileGraph, ServerProfileGraphInput, ServerProfileOwner, ServerProfileSource, UserSession } from '$lib/core'
...
export function canManageProfile(owner: ServerProfileOwner | null, action: `create` | `edit` | `delete`,
  user: Pick<UserSession, `id` | `isSuperadmin` | `isActive`> | null, owners: readonly ProfileOwnerOption[], isPreset = false): boolean {
  if (!user?.isActive || !owner) return false
  if (user.isSuperadmin) return true
  if (isPreset && action !== `create`) return false
...
export function commandRowSummary(command: ServerProfileCommand): { text: string, message: string | null } {
  const delay = `+${command.delayMs}ms`
  if (isMessageCommand(command.commandType)) {
    const message = command.message.trim()
    return { text: delay, message: message ? `"${message}"` : null }
  }
  const details = [delay]
  if (command.commandType === `ban` && command.durationHours != null) details.push(formatDuration(command.durationHours))
  if (command.commandType === `incremental_ban`) {
    const count = command.incrementalBan?.stages.length ?? 0
    details.push(`${count} stage${count === 1 ? `` : `s`}`)
  }
  if ([`kick`, `ban`, `incremental_ban`, `warn`].includes(command.commandType) && command.offenseType?.trim()) {
    details.push(formatOffenseType(command.offenseType))
  }
  return { text: details.join(` / `), message: null }
}
...
function cloneCommand(command: ServerProfileCommand, sortOrder: number): ServerProfileCommand {
  return {
    ...
    ...(command.commandType === `incremental_ban` && command.incrementalBan ? { incrementalBan: structuredClone(command.incrementalBan) } : {}),
    durationHours: command.commandType === `kick` ? null : command.durationHours ?? null,
    message: command.message, offenseType: command.offenseType ?? null
  }
}
...
export function cloneActions(actions: readonly ServerProfileAction[]): ServerProfileAction[] {
  return actions.map((action, sortOrder) => ({
    ...
    showInGameServerActions: action.showInGameServerActions !== false,
...
export function newProfileDraft(owner: ProfileOwner, source: ServerProfileSource | null, existingNames: readonly string[]): ServerProfileGraph {
  return {
    profile: {
      id: 0, owner, name: source ? uniqueCopyName(source.profile.name, existingNames)
        : uniqueNewName(`New profile`, existingNames),
      description: source?.profile.description ?? null, isDefault: false, isMaster: false, isPreset: false
    },
    actions: cloneActions(source?.actions ?? []), servers: [], availableVariables: []
  }
}

export function validateCommandFields(command: { commandType: string, offenseType?: string | null }): void {
  if (!command.commandType.trim()) throw new Error(`Action type is required.`)
  if (![ `server_message`, `admin_message`, `unban` ].includes(command.commandType) && !command.offenseType?.trim()) {
    throw new Error(`Offense type is required.`)
  }
}

export function profileInput(profile: ServerProfileGraph, canPublish = false): ServerProfileGraphInput {
  return {
    ...(canPublish && !profile.profile.isDefault && !profile.profile.isMaster ? { isPreset: profile.profile.isPreset === true } : {}),
    name: profile.profile.name, description: profile.profile.description ?? null,
    serverIds: profile.servers.map(server => server.gameServerId),
    actions: cloneActions(profile.actions).map((action, index) => ({
      ...action,
      ...(profile.actions[index].actionKey ? { actionKey: profile.actions[index].actionKey } : {})
    }))
  }
}

export function profileChanges(profile: ServerProfileGraph, saved: ServerProfileGraph, canPublish = false): ServerProfileGraphInput {
  const current = profileInput(profile, canPublish)
  const previous = profileInput(saved, canPublish)
