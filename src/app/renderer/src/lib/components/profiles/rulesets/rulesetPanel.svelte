<script lang="ts">
  ...
  export function isDirty() {
    return Boolean(draft && (selectedIndex === -1 || JSON.stringify(draft) !== JSON.stringify(ruleset?.rules[selectedIndex]))
  ...
  function add() {
    ...
    draft = { id: 0, rulesetId: ruleset.id, name: `New rule`, description: ``, enabled: false,
      ...
      condition: { mode: `if`, definition: `Player.rank`, operator: `<`, value: 50 }, revision: 1 }
  }
</script>
...
<div class="ruleset-panel">
  ...
  {:else if ruleset}
    ...
    {#if draft}
      ...
      <RuleCard rule={draft} {definitions} {actions} disabled={locked} onToggle={value => { if (draft) draft.enabled = value }} />
      <RuleEditor bind:rule={draft} {definitions} {actions} disabled={locked} />
      {#if draft.id}
    ...
    {:else}
      ...
      <div class="ruleset-panel__rules">
        {#each ruleset.rules as rule, index}
          <RuleCard {rule} {definitions} {actions} disabled={locked} onOpen={() => edit(index)}
