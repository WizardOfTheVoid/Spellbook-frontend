<script lang="ts">
  ...
  function mode(value: string) {
    rule.condition = value === `each` ? { mode: `each`, amount: 1, unit: `minute` } : { mode: `if`, definition: `Player.rank`, operator: `<`, value: 50 }
  }
</script>
...
<section class="rule-editor">
  ...
  {#if rule.condition.mode === `if`}
    ...
    <Select label="Condition" value={rule.condition.definition} options={definitions.map(item => ({ value: item.key, label: item.label }))} {disabled}
      onChange={value => { const next = definitions.find(item => item.key === value)!; rule.condition = { mode: `if`, definition: next.key, operator: next.operators[0]!, value: next.valueType === `number` ? 0 : `` } }} />
    ...
    <Input label={definition?.valueType === `date` ? `Date (ISO with Z or UTC offset)` : definition?.valueType === `time` ? `UTC time (HH:mm)` : `Value${definition?.unit ? ` (${definition.unit})` : ``}`}
