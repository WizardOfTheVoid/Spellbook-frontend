<script lang="ts">
	...
	import { ruleRunDetails } from './ruleDebugDisplay'
	...
	const tabs = [`Tests`, `Queue`, `Timing & producers`, `Rulesets`, `Layout`, `Runs & recording`]
	...
	$: ruleset = state?.ruleset?.snapshot
...
<div class="debug-controls" bind:this={modalRoot}>
	...
	<div class="dialog" role="dialog" aria-modal="true" aria-labelledby="debug-controls-title" tabindex="-1" bind:this={dialog}>
		<header><div><h1 id="debug-controls-title">Debug test controls</h1><p>{state?.enabled ? `Debug enabled` : `Enable Debug in Settings to run tests`} · {state?.core ? `Core connected` : `Core unavailable`}</p></div><button type="button" on:click={async () => {
			onClose()
			await tick()
			openCredits(returnFocus)
		}}>Credits</button><button type="button" bind:this={closeButton} on:click={onClose}>Close</button></header>
		{#if api && state}
			...
			<div class="content">
				...
				{#if tab === `Tests`}
					<p>Tests arm automatically with Debug. Each physical press fires once while game-focused. Release the trigger to let normal input gates settle. Press a loop key again to stop it. Ctrl+F5 toggles Debug; F6 fades the HUD.</p>
				...
				{:else if tab === `Rulesets`}
					{#if state.ruleset?.error}<p class="error">{state.ruleset.error}</p>{/if}
					{#if ruleset}
						<p>Server {ruleset.gameServerId} · {ruleset.rules.length} enabled rules</p>
						{#each ruleset.rules as rule (rule.id)}
							<div class="run"><strong>{rule.name} · {rule.status}</strong><p>{rule.profileName} · {rule.reason}</p>
								{#if rule.lastRun}<p>Last run #{rule.lastRun.id}: {rule.lastRun.status}{ruleRunDetails(rule.lastRun) ? ` · ${ruleRunDetails(rule.lastRun)}` : ``}</p>{/if}
							</div>
						{:else}<p>No enabled rules attached to this server.</p>{/each}
					{:else}<p>Waiting for a current server and ruleset diagnostics.</p>{/if}
