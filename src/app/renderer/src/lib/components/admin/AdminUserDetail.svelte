<script lang="ts">
	...
	function formatDate(value: string | null, now: Date): string {
		return value ? formatShortRelativeDateTime(value, now) : `Never`
	}
</script>
...
<div class="admin-user-profile">
	...
	<section class="profile-section" aria-label="Activity">
		...
		<TileGrid columns={2}>
			<Tile title="Last active" subtitle={formatDate(user.lastActive, now)} tooltip={user.lastActive ? formatFullDateTime(user.lastActive) : null} icon="fa-clock" iconTone="info" />
			<Tile title="Last login" subtitle={formatDate(user.lastLogin, now)} tooltip={user.lastLogin ? formatFullDateTime(user.lastLogin) : null} icon="fa-right-to-bracket" iconTone="accent" />
			<Tile title="SpellBook version" subtitle={user.currentVersion ?? `Not reported`} icon="fa-code-branch" iconTone="info" />
		</TileGrid>
	</section>
	...
	<footer class="user-metadata">
		<span>Joined <strong use:tooltipAction={formatFullDateTime(user.createdAt)}>{formatDate(user.createdAt, now)}</strong></span>
		<span>Updated <strong use:tooltipAction={formatFullDateTime(user.updatedAt)}>{formatDate(user.updatedAt, now)}</strong></span>
	</footer>
</div>
