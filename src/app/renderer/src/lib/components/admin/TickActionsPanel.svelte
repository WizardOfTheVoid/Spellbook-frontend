<script lang="ts">
  ...
  function logTime(value: string): string {
	...
	rememberNavigation(`tickLogFilter`, () => selectedLogLevels, value => { selectedLogLevels = value }, () => null)
</script>
    ...
    <div class="tick-action__controls">
      <span class="tick-action__control-group">
        <Button
          ...
          icon="fa-play"
          variant="primary"
          tooltip={selected.action === `cleanup` ? `Remove tick run logs older than seven days.` : null}
        ...
        <Button
          ...
          tooltip={selected.supportsPause ? null : `This action finishes in one pass.`}
  ...
  <TileGrid columns={1}>
    {#each actions as action (action.action)}
      <Tile
        ...
        icon={action.action === `enrichEmptyPlayers` ? `fa-user-plus` : action.action === `normalizePlayerNames` ? `fa-font` : action.action === `cleanup` ? `fa-broom` : action.action === 'leaderboard' ? 'fa-ranking-star' : 'fa-server'}
        tone={statusTone(action)}
        onClick={() => void navigation.visit(() => openAction(action))}
      />
    {/each}
  </TileGrid>
{:else}
