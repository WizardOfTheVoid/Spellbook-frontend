<script lang="ts">
	...
	import { rememberNavigation } from "$lib/navigation/navigation"
	...
	import { openAdminUserMenu } from "./adminUserBan";

	export let active = false;
	export let onOpenUser: (userId: number) => void;
	export let onBanUser: (user: AdminUserRecord) => void;
	...
	async function loadUsers(): Promise<void> {
		...
		try {
			...
			users = loadedUsers.sort((a, b) =>
				(Date.parse(b.lastActive ?? ``) || 0) - (Date.parse(a.lastActive ?? ``) || 0)
				|| (b.currentVersion ?? ``).localeCompare(a.currentVersion ?? ``, undefined, { numeric: true })
				|| getAdminName(a).localeCompare(getAdminName(b), undefined, { sensitivity: `base` }))
			loaded = true;
		} catch (error) {
			notifyError(message(error, "Users request failed."), {
				dedupeKey: "admin:users",
			});
		} finally {
			loading = false;
		}
	...
	rememberNavigation(`adminUserSearch`, () => search, value => { search = value }, () => null)
</script>

<div class="admin-users grid-stack gap-1">
		...
		>
			<svelte:fragment slot="leading">
				<span class="admin-avatar">
					{#if user.avatarUrl}<img src={user.avatarUrl} alt="" />{:else}<Icon
							name="fa-user"
							size="md"
							tone="muted"
						/>{/if}
			...
			<svelte:fragment slot="trailing">
				<span class="admin-version"
					>{user.currentVersion ?? `Not reported`}</span
				>
				<span
					class:admin-status--banned={accountStatus(user) === "suspended"}
					class="admin-status"
				>
