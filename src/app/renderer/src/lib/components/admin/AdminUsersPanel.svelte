...
<div class="admin-users grid-stack gap-100">
	...
	{#each filteredUsers as user (user.id)}
		<ListRow
			...
			onContextMenu={event => openAdminUserMenu(event, { actor: $authState.user, user, onOpenUser, onBan: onBanUser })}
		>
			...
			<svelte:fragment slot="trailing">
				<span class="admin-version">{user.currentVersion ?? `Not reported`}</span>
