<script lang="ts">
	...
	import TagTypesPanel from "./definitions/tagTypesPanel.svelte"
	import { getAdminName } from "@spellbook/shared/adminName.js"

  import ActionAdminPanel from "./actions/actionAdminPanel.svelte"
	...
	import AdminDiscordPanel from "./AdminDiscordPanel.svelte"
	import DiscordQueuePanel from "./DiscordQueuePanel.svelte"
	...
	import UxUiPanel from './uxUiPanel.svelte'
	...
	async function setWantedPermission(enabled: boolean): Promise<void> {
		...
		try {
			...
			notifySuccess(`Wanted creation ${enabled ? "enabled" : "disabled"} for ${getAdminName(selectedUser)}.`)
		} catch (error) {
	...
	async function setAccountEnabled(enabled: boolean): Promise<void> {
		...
		const userId = selectedUser.id
		const accountName = getAdminName(selectedUser)
	...
	async function leaveDefinitions(): Promise<void> {
		if (await unsavedChanges.canLeave()) await navigation.visit(() => { view = `definitions` })
	}

	function back(): void {
		if (view === `tag-types`) { void leaveDefinitions(); return }
		if (view === `discord-queue`) {
			view = `discord`
			return
		}
	...
	rememberNavigation(`admin`, () => ({ view, selectedUser, selectedTeamId, selectedTickAction }), async state => {
		({ view, selectedUser, selectedTeamId, selectedTickAction } = state)
		if (view === `dev-ui` && import.meta.env.DEV) devComponent = (await import(`$lib/components/dev/UiGalleryPanel.svelte`)).default
	}, state => [state.view, state.selectedUser?.id, state.selectedTeamId, state.selectedTickAction])
</script>
	...
	<section {hidden} class="panel-view admin-panel" aria-label="Admin">
		<PanelHeader
			...
			leadingLabel={view === "user" ? "Back to users"
				: view === `tag-types` ? `Back to definitions`
				: view === `discord-queue` ? `Back to Discord`
		...
		{#if view === "root"}
			<div class="admin-root panel-subview" use:navigationScroll={`admin-root`}>
				<TileGrid columns={1}>
		...
		{:else if view === "users"}
			<div class="admin-users panel-subview" use:navigationScroll={`admin-users`}>
				<AdminUsersPanel bind:this={usersPanel} active={isActive} onOpenUser={(userId) => void navigation.visit(() => openUser(userId))} onBanUser={openBanUser} />
			</div>
		{:else if view === `definitions`}
      <div class="admin-definitions panel-subview" use:navigationScroll={`admin-definitions`}><TileGrid columns={1}><Tile title="Tag: Types" subtitle="Tag names, variants and message wording" icon="fa-tags" onClick={() => void navigation.visit(() => { view = `tag-types` })} /></TileGrid></div>
    {:else if view === `tag-types`}
      <div class="admin-definitions panel-subview" use:navigationScroll={`admin-tag-types`}><TagTypesPanel /></div>
    {:else if view === `actions`}<ActionAdminPanel />
		{:else if view === "teams" || view === "team"}
			<div class="admin-teams panel-subview" use:navigationScroll={`admin-teams`}>
				<AdminTeamsPanel
					active={isActive}
					{selectedTeamId}
					onSelectedTeam={(teamId) => void navigation.visit(() => {
						selectedTeamId = teamId
		...
		{:else if view === "audit-logs"}
			<div class="admin-audit-logs panel-subview" use:navigationScroll={`admin-audit-logs`}><AuditLogsPanel active={isActive} /></div>
		{:else if view === `discord`}
			<div class="admin-discord panel-subview" use:navigationScroll={`admin-discord`}><AdminDiscordPanel onOpenQueue={() => void navigation.visit(() => { view = `discord-queue` })} /></div>
		{:else if view === `discord-queue`}
			<div class="admin-discord panel-subview" use:navigationScroll={`admin-discord`}><DiscordQueuePanel active={isActive && !hidden} /></div>
		{:else if view === "integration-tests"}
			<div class="admin-integration-tests panel-subview" use:navigationScroll={`admin-integration-tests`}><IntegrationTestsPanel /></div>
		{:else if view === "notification-tests"}
			<div class="admin-notification-tests panel-subview" use:navigationScroll={`admin-notification-tests`}><NotificationTestsPanel /></div>
		{:else if view === "tick-actions"}
			<div class="admin-tick-actions panel-subview" use:navigationScroll={`admin-tick-actions`}>
				<TickActionsPanel active={isActive} bind:selectedAction={selectedTickAction} />
			</div>
		{:else if view === `ux-ui`}
			<div class="admin-ux-ui panel-subview" use:navigationScroll={`admin-ux-ui`}><UxUiPanel /></div>
		{:else if view === "dev-grid" || view === "dev-ui"}
			<div class="admin-dev panel-subview" use:navigationScroll={`admin-dev`}>
				{#if view === "dev-grid"}
		...
		{:else if selectedUser}
			<div class="admin-user panel-subview" use:navigationScroll={`admin-user`}>
				<AdminUserDetail
					user={selectedUser}
					{saving}
