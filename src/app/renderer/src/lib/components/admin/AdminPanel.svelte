...
{:else}
	<section {hidden} class="panel-view admin-panel" aria-label="Admin">
		...
		{:else if view === `discord`}
			<div class="admin-discord panel-subview"><AdminDiscordPanel onOpenQueue={() => view = `discord-queue`} /></div>
		{:else if view === `discord-queue`}
			<div class="admin-discord panel-subview"><DiscordQueuePanel active={isActive && !hidden} /></div>
