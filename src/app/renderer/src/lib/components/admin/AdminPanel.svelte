<script lang="ts">
	...
	async function setAccountEnabled(enabled: boolean): Promise<void> {
		...
		try {
			...
			notifySuccess(`${selectedUser?.displayName ?? "Account"} ${enabled ? "enabled" : "suspended"}.`)
		} catch (error) {
	...
	<section {hidden} class="panel-view admin-panel" aria-label="Admin">
		...
		{:else if view === "users"}
			<div class="admin-users panel-subview">
				<AdminUsersPanel active={isActive} onOpenUser={(userId) => void openUser(userId)} />
			</div>
		...
		{:else if selectedUser}
			<div class="admin-user panel-subview">
				<AdminUserDetail
					user={selectedUser}
					{saving}
					accountControlDisabled={selectedUser.id === $authState.user?.id}
