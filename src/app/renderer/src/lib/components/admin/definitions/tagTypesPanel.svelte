<script lang="ts">
	import { messageFactValues } from "@spellbook/shared/actions/messageFacts"
	import { exampleMessageFacts } from "$lib/components/messages/messageLegend"
	...
	$: examples = definedMessageValues(
		{
			...messageFactValues(exampleMessageFacts),
			user: `Samwise`,
			admin: `Admin`,
			playfab: `PLAYER_1`,
			duration: `24`,
							...
							{#if row.group === `tag`}
								{#if seed(row).alt}
									<Checkbox
										...
										description={seed(row).alt === `days` ? `Elapsed days since last login.` : `Ordinal count: 1st, 2nd, 3rd, 11th.`}
										checked={row.alt === seed(row).alt}
										onChange={(enabled) =>
											change(row, { alt: enabled ? seed(row).alt : null })}
									/>
								{/if}
