<script lang="ts">
	...
	import { navigation, navigationState } from "$lib/navigation/navigation"
	import { tooltip } from '$lib/utils/tooltip'
	import IconButton from "./IconButton.svelte";

	export let title: string;
	export let eyebrow: string | null = null;
	export let eyebrowTooltip = ``
	...
	export let historyBack = true
	$: useHistory = historyBack && variant === `panel`
</script>

<header class="panel-header panel-header--{variant}">
	{#if (leadingIcon && onLeading) || (useHistory && $navigationState.canBack)}
		<IconButton
			icon={leadingIcon ?? `fa-arrow-left`}
			ariaLabel={useHistory ? `Back` : leadingLabel}
			tooltip={useHistory ? `Back` : leadingLabel}
			navigationBack={useHistory}
			onClick={() => useHistory ? void navigation.back(onLeading ?? undefined) : onLeading?.()}
			size="lg"
			disabled={leadingDisabled}
		/>
	{/if}
	<div class="panel-header__identity">
		{#if eyebrow}<p class="panel-header__eyebrow"><span use:tooltip={eyebrowTooltip}>{eyebrow}</span></p>{/if}
		<svelte:element
