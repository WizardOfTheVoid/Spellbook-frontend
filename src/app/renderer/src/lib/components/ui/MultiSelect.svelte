<script lang="ts">
	...
	$: accessibleLabel = ariaLabel?.trim() || label?.trim() || placeholder?.trim() || `Select options`
...
>
	{#if hasLabel}<label id={`${controlId}-label`} for={`${controlId}-search`}>{label}</label>{/if}
	<div
		...
		aria-labelledby={hasLabel && !ariaLabel ? `${controlId}-label` : undefined}
		aria-label={!hasLabel || ariaLabel ? accessibleLabel : undefined}
