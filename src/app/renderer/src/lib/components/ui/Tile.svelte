<script lang="ts">
	...
	export let iconType: "solid" | "regular" | "light" | "thin" | "brands" =
		"light";
...
{#if onClick}
	...
	>
		...
		<span class="ui-tile__copy">
			<strong
				>{title}{#if suffix}
					<span class="ui-tile__suffix">{suffix}</span>{/if}</strong
			>
			{#if subtitle}<span>{subtitle}</span>{/if}
		</span>
		{#if badge > 0}<span
				class="ui-tile__badge"
				aria-label={`${badge} pending requests`}>{badge}</span
			>{/if}
...
<style lang="scss">
	...
	.ui-tile__suffix {
		...
		background: rgbaa(var(--color-dark-tertiary), 0.1);
