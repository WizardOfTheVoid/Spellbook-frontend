			...
			{#each level.items as item, index}
				{#if item.children?.length || item.loadChildren}
					...
					>
						...
						<span>{item.name} <CountBadge count={item.badge ?? 0} />{#if item.suffix} <small>{item.suffix}</small>{/if}{#if item.suffixIcon} <span class="personal-icon" aria-label="Personal profile"><Icon name={item.suffixIcon} size="sm" tone="muted" /></span>{/if}</span>
