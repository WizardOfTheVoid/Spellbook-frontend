<script lang="ts">
	...
	export let view: `debug` | `onboarding` = `debug`
...
<div class="beta-modal" bind:this={modalRoot}>
	...
	>
		<header>
			<IconButton icon="fa-arrow-left" ariaLabel="Back" tooltip="Back" navigationBack={true}
				onClick={() => void navigation.back(view === `onboarding` ? () => { view = `debug` } : onClose)} />
			<span class="beta-mark"><Icon name="fa-flask" size="xlg" /></span>
			<IconButton icon="fa-xmark" ariaLabel="Close beta welcome" tooltip="Close"
				bind:element={closeButton} onClick={onClose} />
		</header>
		<nav class="help-tabs" aria-label="Help sections">
			<button
				...
				on:click={() => void navigation.visit(() => { view = `debug` })}>Debug & help</button
			>
			<button
				type="button"
				class:active={view === `onboarding`}
				aria-pressed={view === `onboarding`}
				on:click={() => void navigation.visit(() => { view = `onboarding` })}>Onboarding</button
			>
		</nav>
		{#if view === `onboarding`}
			<div>
			...
			<TodoChecklist
				onNavigate={(page) => void navigation.visit(async () => {
					onClose()
		...
		{:else}
			<div class="beta-copy">
				<h1 id="beta-title">Welcome to SpellBook beta!</h1>
				...
				<p>
					When reporting bugs; Include what happened, the steps to reproduce it,
					and any screenshots or error messages.
				</p>
				<p>Press <strong><KeybindValue name="overlayKey" /></strong> to open or close SpellBook.</p>
			</div>
			<div class="discord-section">
				...
				<div class="discord-links">
					...
					<Button
						...
						onClick={() => void navigation.visit(() => { view = `onboarding` })}
					/>
				</div>
			</div>
			<footer>
