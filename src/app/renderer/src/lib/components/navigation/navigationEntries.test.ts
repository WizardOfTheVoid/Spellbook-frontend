...
test(`shows Analysis with a security icon only to superadmins`, async () => {
  const entries = await loadCreateNavigationEntries()
  assert.equal(entries(false, 0).some(entry => entry.page === `analysis`), false)
  assert.deepEqual(entries(true, 0).find(entry => entry.page === `analysis`), {
    page: `analysis`, label: `Analysis`, icon: `fa-user-shield`
  })
})
