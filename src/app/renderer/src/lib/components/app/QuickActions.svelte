<script lang="ts">
	...
	$: blocked = $consoleSetup.commandsBlocked
	$: if (blocked && messageKind) void closeComposer()
	...
	async function sendMessage(): Promise<void> {
		if (blocked || !messageKind || !draft.trim() || sending) return;
...
>
	<button
		...
		disabled={blocked}
		use:tooltipAction={blocked ? $consoleSetup.commandIssue ?? `` : `Say something as you with red text.`}
	...
	<button
		...
		disabled={blocked}
		use:tooltipAction={blocked ? $consoleSetup.commandIssue ?? `` : `Say something as server in yellow text.`}
	...
	<button
		...
		use:tooltipAction={blocked ? $consoleSetup.commandIssue ?? `` : antiAfkControl.tooltip}
	...
	<button
		...
		disabled={blocked}
		use:tooltipAction={blocked ? $consoleSetup.commandIssue ?? `` : `Constant surveillance, may cause lag.`}
