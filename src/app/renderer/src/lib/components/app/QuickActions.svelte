<script lang="ts">
	...
	import { messageLegendItems } from "$lib/components/messages/messageLegend"
	...
	$: messageContext = {
		...
		admin: getAdminName(user),
		serverName:
			gameServer?.displayName?.trim() || gameServer?.name || serverName,
		serverSlot: gameServer?.maxPlayers,
	...
	$: variableTags = quickActionTagItems(
		activeProfileVariables(activeProfile),
		activeProfile?.variables ?? [],
		messageContext,
	);
	$: tags = [...messageLegendItems($tagTypes, messageContext, { player: false, authoring: false }),
		...variableTags.filter(item => !contextualMessageKeys.has(item.tag.slice(1, -1))).map(item => ({
			...item, group: `Server variables`, example: resolveQuickActionMessage(item.tag, messageContext)
		}))]
	...
	onMount(() => {
		...
		const clock = setInterval(() => { if (messageKind) now = new Date().toISOString() }, 1000)
	...
	async function openComposer(
		...
		try {
			await tagTypes.load()
			const currentFacts = gameServer?.id ? await actionsApi<MessageFacts>(`messageFacts`, { id: gameServer.id }) : {}
			if (version !== composerVersion) return
			facts = currentFacts
			now = new Date().toISOString()
			if (version !== composerVersion || blocked || composerServerId !== serverExternalId) return
	...
	async function sendMessage(): Promise<void> {
		...
		try {
			if (gameServer?.id && usesServerMessageFacts([{ message: draft }])) {
				facts = await actionsApi<MessageFacts>(`messageFacts`, { id: gameServer.id })
			}
			if (version !== composerVersion || blocked || composerServerId !== serverExternalId) return
			const resolved = resolveQuickActionMessage(draft, { ...messageContext, ...facts, now: new Date().toISOString() }, kind)
