<script lang="ts">
	...
	async function loadPlayerState(showLoading = true): Promise<void> {
		...
		try {
			...
			if (destroyed || session !== profileSession || playfabId !== loadedPlayfabId) return
	...
	async function refreshStalePlayFab(playfabId: string): Promise<void> {
		...
		try {
			...
			if (!destroyed && session === profileSession && playfabId === loadedPlayfabId) {
				notifyWarning(
			...
			if (session === profileSession && playfabId === loadedPlayfabId) playFabLoading = false
		}
	}
	...
	async function loadActionContext(): Promise<void> {
		...
		const current = () => request === actionRequest && externalId === currentServerExternalId
			&& playfabId === currentPlayer.playfabId && profilePreferencesState.isCurrent(session)
		actionLoading = true
	...
	async function runAction(action: ServerProfileAction): Promise<void> {
		...
		try {
			const result = await executeProfileAction(action, {
				...
				beforeExecute: profileExecutionGuard($authState.user.id, activeProfile?.gameServer?.externalId, profileActionRequiresGameProcess(action), action.id),
			});
	...
	rememberNavigation(`playerDetail`, () => ({ actionMode, selectedAction, selectedUser, addingNote, seedActionId, notesMode }), async state => {
		({ actionMode, selectedAction, selectedUser, addingNote, seedActionId, notesMode } = state)
		if (actionMode) await loadActionContext()
	}, state => [state.actionMode, state.selectedAction?.id, state.selectedUser?.id, state.addingNote, state.notesMode])
</script>
...
>
	...
	>
		<svelte:fragment slot="title">
			<PlayerPresenceName name={playerDisplayName} isOnline={dbProfile?.presence?.isOnline ?? currentPlayer.isOnline} presence={dbProfile?.presence} />
		</svelte:fragment>
	...
	<div class="player-detail__body" use:navigationScroll={`playerDetail`}>
		...
		{#if actionMode}
			...
			<ProfileActionList
				...
				actions={graph.actions.filter(action => action.isEnabled && action.actionDomain === `player`)}
			...
			/>
			{:else}
				<ProfileActionList title="In-game actions" actions={[]} loading={actionLoading} onRun={() => {}} />
			{/each}
		...
		{:else}
			...
			<PlayerActionsSection
				...
				loadUnbanActions={action => loadProfileActionMenu({ playerId: action.playerId, playfabId: currentPlayer.playfabId, name: playerDisplayName }, {
					commandType: `unban`, relatedActionId: action.id, onComplete: loadPlayerState,
				})}
