<script lang="ts">
	...
	import PlayerTimeline from "./playerTimeline/playerTimeline.svelte"
	...
	import { copyProfileLink } from "./playerInfinityMenu";
	...
	async function openTimelineTarget(target: PlayerTimelineTarget): Promise<void> {
		if (target.type === `server`) {
			onOpenServer(target.id)
			return
		}
		if (!dbProfile) return
		try {
			const action = dbProfile.actions.find(item => item.id === target.id)
				?? parsePlayerAction(await unwrap<unknown>(
					await getServerApi().playerActions.get(dbProfile.player.id, target.id),
					`Player action request failed.`
				))
			selectAction(action)
		} catch (error) {
			notifyError(error instanceof Error ? error.message : `Player action could not be opened.`)
		}
	}
...
>
	...
	>
		...
		<svelte:fragment slot="trailing">
			{#if !actionMode && !notesMode && !selectedAction && !selectedUser}
				<IconButton
					icon="fa-link"
					ariaLabel="Copy profile link"
					tooltip="Copy profile link"
					size="lg"
					onClick={() => void copyProfileLink(currentPlayer.playfabId)}
				/>
			{/if}
			{#if selectedAction}
				<Tag
	...
	<div class="player-detail__body" use:navigationScroll={`playerDetail`}>
		...
		>
			...
			{:else if infoTab === `history`}
				{#if dbProfile?.player.id}
					<PlayerTimeline playerId={dbProfile.player.id} actions={dbProfile.actions} presence={dbProfile.presence} onOpenTarget={(target) => void openTimelineTarget(target)} />
				{:else}
					<p class="player-detail__notice">Loading player timeline...</p>
				{/if}
