<script lang="ts">
	...
	export let onOpenServer: (serverId: number) => void
	...
	$: completedByServer = new Map(
		(detail?.automaticActions ?? []).map((action) => [
			action.gameServerId,
			action,
		]),
	);
	...
	async function revert(): Promise<void> {
		if (
			!source ||
			viewState.mutation ||
			!window.confirm(`Revert the global ban for ${playerName}?`)
		)
			return;
	...
	async function remove(): Promise<void> {
		if (
			!detail?.canRemove ||
			viewState.mutation ||
			!window.confirm(`Remove ${playerName} from Wanted?`)
		)
			return;
		const selectedGeneration = contextGeneration;
	...
	function displayServer(action: PlayerAction): string {
		return (
			action.gameServer?.displayName?.trim() ||
			action.gameServer?.name?.trim() ||
			(action.gameServerId ?
				`Server #${action.gameServerId}`
			:	`Community Hivemind Actions`)
		);
	}

	function coverageStatus(action: PlayerAction): string {
		if (!source) return `Unavailable`;
		if (source.actionType === `unban`)
			return completedByServer.has(action.gameServerId) ? `Reverted` : (
					`Pending`
				);
		if (source.actionType === `mock`) return `Reached`;
		return action.id === source.id ? `Original action` : `Reached`;
	}
</script>
...
{:else}
	<section
		class="panel-view panel-view--sub wanted-detail"
		aria-label="Wanted player detail"
	>
		<WantedAmbient />
		<PanelHeader
			title={playerName}
			...
			<svelte:fragment slot="title">
				<PlayerPresenceName
					...
					isOnline={detail?.presence?.isOnline ??
						detail?.player.isOnline ??
						player.isOnline}
					presence={detail?.presence}
				/>
			</svelte:fragment>
			<svelte:fragment slot="trailing">
				<Button label="Profile" icon="fa-user" onClick={onOpenProfile} />
				{#if player.dbId}<Tag
						label="Notes"
						suffix={`(${noteCount})`}
						icon="fa-note-sticky"
		...
		<div
			class="wanted-detail__body"
			aria-busy={viewState.loading}
			use:navigationScroll={`wantedDetail`}
		>
			{#if !detail && viewState.loading}
				<EmptyState
					title="Loading wanted player"
					message="Fetching the current Wanted action and server coverage."
				/>
			{:else if !detail}
				<EmptyState
					...
					message={viewState.error ??
						(player.dbId ?
							"Wanted detail could not be loaded."
						:	"This player has no database ID.")}
				/>
			...
			{:else}
				...
				<div class="wanted-detail__actions">
					{#if detail.canRevert && source?.actionType === `ban`}
						<Button
							label={viewState.mutation === `revert` ?
								"Reverting..."
							:	"Revert global ban"}
							icon="fa-rotate-left"
							disabled={viewState.mutation !== null}
							onClick={() => void revert()}
						/>
					{/if}
					{#if detail.canRemove}
						<Button
							label={viewState.mutation === `remove` ?
								"Removing..."
							:	"Remove player"}
							icon="fa-user-minus"
							variant="danger"
				...
				<div class="wanted-detail__grid">
					<article class="wanted-detail__card">
						...
						{#if source}
							<dl>
								<div>
									<dt>Action</dt>
									<dd>{actionLabel(source)}</dd>
								</div>
								<div>
									<dt>Reason</dt>
									<dd>{source.reason?.trim() || "No reason recorded."}</dd>
								</div>
								<div>
									<dt>Author</dt>
									<dd>{actionAuthor(source)}</dd>
								</div>
								<div>
									<dt>Origin</dt>
									<dd>{displayServer(source)}</dd>
								</div>
								<div>
									<dt>Created</dt>
									<dd title={formatFullDateTime(source.createdAt)}>
										{formatShortRelativeDateTime(source.createdAt)}
									</dd>
								</div>
							</dl>
						{:else}
					...
					<article class="wanted-detail__card">
						...
						<dl>
							<div>
								<dt>PlayFab ID</dt>
								<dd>{detail.player.playfabId}</dd>
							</div>
							<div>
								<dt>Database ID</dt>
								<dd>#{detail.player.id}</dd>
							</div>
							<div>
								<dt>Last seen</dt>
								<dd>
									{detail.player.lastSeen ?
										formatFullDateTime(detail.player.lastSeen)
									:	"Unavailable"}
								</dd>
							</div>
						</dl>
				...
				<article class="wanted-detail__card">
					...
					<div class="wanted-detail__coverage">
						...
						{:else}
							<p>
								{source?.actionType === `mock` ?
									"No server has executed this mock action yet."
								:	"No server has executed this action yet."}
							</p>
						{/each}
					</div>
				</article>
				...
				<article class="wanted-detail__card">
					...
					<div class="wanted-detail__coverage">
						{#each detail.automaticActions as action (action.id)}
							<div class="wanted-detail__coverage-row">
								<div>
									<strong>{displayServer(action)}</strong><small
										>{actionLabel(action)} · {formatFullDateTime(
											action.createdAt,
										)}</small
									>
								</div>
						...
						{:else}
							<p>No wanted action was done for the source.</p>
						{/each}
					</div>
				</article>
			{/if}
...
<style lang="scss">
	...
	.wanted-detail > :global(*) {
		position: relative;
		z-index: 1;
	}
	.wanted-detail > :global(.wanted-ambient) {
		position: absolute;
		z-index: 0;
	}
