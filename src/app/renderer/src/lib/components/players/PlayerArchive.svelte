<script lang="ts">
	...
	$: advancedFilterCount = countAdvancedPlayerFilters(filters, mode, source)
	...
	$: if (active) {
		if (mode === `live` && preparedRoster && !backendFiltered) {
			...
			const result = pagePreparedRoster(preparedRoster, page)
	...
	function resetFilters(): void {
		queryDebouncer.cancel()
		const reset = createPlayerArchiveResetState(source)
		page = reset.page
	...
	rememberNavigation(`playerArchive`, () => ({ page, sessionPage, searchInput, search, filters, queryFilters, activeChipIds, advancedFiltersOpen }), value => {
		({ page, sessionPage, searchInput, search, filters, queryFilters, activeChipIds, advancedFiltersOpen } = value)
		queryDebouncer.cancel()
	}, () => null)
</script>
...
<div class="player-archive">
	<div class="player-archive__filters">
		...
		{#if advancedFiltersOpen}
			<AdvancedPlayerFilters {filters} {mode} {source} onChange={handleFilters} />
		{/if}
	...
	<div class="player-archive__body" use:navigationScroll={`playerArchive`} aria-busy={state === "loading"}>
		{#if state === "loading"}
