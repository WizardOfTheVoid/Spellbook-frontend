...
test(`incorrect bindings disable in-game actions while leaving player navigation available`, async () => {
  ...
  assert.equal(menu.items[2]?.disabled, true)
  assert.ok(menu.items[2]?.tooltip)
