...
test(`loads configured actions only when the in-game submenu is opened`, async () => {
 ...
 const children = await menu.items.find(item => item.loadChildren)!.loadChildren?.()
...
test(`Wanted menus permit local unban choices`, async () => {
 const { createPlayerInfinityMenu } = await loadPlayerInfinityMenu()
 const menu = createPlayerInfinityMenu(target(), {
  wanted: { actionType: `ban`, sourceActionId: 9, sourceAuthorId: 3, viewerId: 3, isSuperadmin: false },
  loadProfileActions: async excludeUnban => { assert.equal(excludeUnban, false); return [] },
 })
 assert.deepEqual(await menu.items.find(item => item.loadChildren)!.loadChildren?.(), [])
})
...
test(`keeps requests usable while the game is unavailable`, async () => {
	const { createPlayerInfinityMenu } = await loadPlayerInfinityMenu()
	const menu = createPlayerInfinityMenu(target(), { gameAvailable: false })

	assert.equal(menu.items.find(item => item.loadChildren)?.disabled, false)
})
...
test(`local binding problems do not block remote requests or navigation`, async () => {
  const { createPlayerInfinityMenu } = await import(`./playerInfinityMenu`)
  const menu = createPlayerInfinityMenu({ playerId: 1, name: `Player`, playfabId: `id`, onOpen: () => {} },
    { gameAvailable: true, commandsBlocked: true })
  assert.equal(menu.items.find(item => item.loadChildren)?.disabled, false)
  assert.notEqual(menu.items[0]?.disabled, true)
  assert.notEqual(menu.items[1]?.disabled, true)
})

test(`copy profile link writes the canonical HTTPS player URL`, async () => {
  const { createPlayerInfinityMenu } = await import(`./playerInfinityMenu`)
  ...
  try {
    const menu = createPlayerInfinityMenu({ ...target(), playfabId: `ABC_123` })
    await runItem(itemNamed(menu.items, `Copy profile link`))
    assert.equal(copied, `https://chivalry2.dev/sb/players/ABC_123`)
  } finally {
