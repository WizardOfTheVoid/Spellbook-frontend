<script lang="ts">
	...
	$: resolvedDisplayName =
		!$gameState.available ? `Game status unavailable`
		: !gameRunning ? `Game not running`
		: isMainMenu ? `Main Menu`
		: !inGameServer || !hasCurrentSnapshot ? `No current server`
		: activeProfile?.gameServer ? getServerLabel(activeProfile.gameServer)
		: serverDisplayName;
	$: headerTitle =
	...
	$: {
		...
		if (summaryKey !== lastSummaryKey) {
			...
			onSummaryChange({
				serverExternalId: inGameServer ? serverExternalId : null,
				serverName: inGameServer ? serverName : resolvedDisplayName,
				serverAddress: inGameServer ? serverAddress : null,
	...
	function handleArchiveResult(result: PlayerArchiveResult): void {
		...
		if (result.refreshedAt)
			lastPlayerRefresh = formatTime(new Date(result.refreshedAt));
