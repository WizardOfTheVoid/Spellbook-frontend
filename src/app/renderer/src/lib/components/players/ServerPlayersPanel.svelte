<script lang="ts">
	...
	$: profileKey = `${serverExternalId ?? ""}:${$gameServerRevision}:${$profilePreferencesState.userId}:${$profilePreferencesState.revision}`
	$: if (!isActive) activeProfileKey = ``
	// The stored pretty name wins once the active profile resolves; the Core-derived name covers the gap.
	$: resolvedDisplayName =
		isMainMenu ? `Main Menu`
		: activeProfile?.gameServer ? getServerLabel(activeProfile.gameServer) : serverDisplayName;
	...
	async function refreshActiveProfile(): Promise<void> {
		...
		const externalId = serverExternalId
		const session = profilePreferencesState.captureSession()
		const current = () => request === profileRequest && externalId === serverExternalId && profilePreferencesState.isCurrent(session)
		try {
			const result = await fetchActiveServerProfile(externalId)
			if (!current()) return
	...
	rememberNavigation(`serverActions`, () => serverActionsMode, async value => {
...
>
	...
	{:else}
		<div class="meta">
			<TileGrid columns={2}>
				{#if activeProfile?.ownerTeamId}
					<Tile title="Team" subtitle="Open the owner team" icon="fa-users" onClick={() => onOpenTeam(activeProfile!.ownerTeamId!)} />
				{/if}
				<Tile
					title="Actions"
