<script lang="ts">
  ...
  let appliedSeedKey = ``
  let now = new Date()
  rememberNavigation(`noteEditor`, () => ({ content, scope, editingNoteId, editContent, editScope }), state => {
    ({ content, scope, editingNoteId, editContent, editScope } = state)
  }, () => null)
  ...
  const author = (note: PlayerNote) => getAdminName(note.author, note.author.playfabId?.trim() || `#${note.author.id}`)
