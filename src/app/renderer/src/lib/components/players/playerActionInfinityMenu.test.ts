...
test(`incorrect bindings disable unban while keeping player notes available`, () => {
  const menu = createPlayerActionInfinityMenu(action(), [action()], { gameAvailable: true, commandsBlocked: true }, now)
  assert.equal(menu.items[0]?.disabled, true)
  assert.ok(menu.items[0]?.tooltip)
