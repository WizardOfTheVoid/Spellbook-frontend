...
import { createPlayerWebUrl } from '@spellbook/shared/appLinks'
import {
	openInfinityMenu,
...
export function createPlayerInfinityMenu(
	...
	items.push(
		...
		},
		{
			...
			icon: `fa-link`,
			action: () => copyProfileLink(target.playfabId),
		},
		{
...
export async function copyProfileLink(playfabId: string): Promise<void> {
	try {
		await navigator.clipboard.writeText(createPlayerWebUrl(playfabId))
		notifySuccess(`Profile link copied.`)
	} catch {
		notifyError(`Could not copy the profile link.`)
	}
}
