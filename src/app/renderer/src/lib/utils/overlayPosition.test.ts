...
test(`prefers below an anchor when the requested side overflows`, async () => {
	const { placeAnchoredOverlay } = await loadOverlayPosition()
...
test(`tries the left side before clamping over the anchor`, async () => {
	const { placeAnchoredOverlay } = await loadOverlayPosition()
	assert.deepEqual(placeAnchoredOverlay(
		anchor({ left: 430, top: 30, right: 470, bottom: 370 }),
