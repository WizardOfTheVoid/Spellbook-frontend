...
test(`renderer resolves optional variants and readable offense wording`, () => {
  assert.equal(resolveMessageTemplate(`[offenses_alt] [offense_type] [offense_type_pt]`, {
    offenses: `12`, offenseType: `verbal_abuse`, actionType: `ban`
  }), `12th Verbal Abuse banned`)
})
...
test(`resolves built-in message tags`, () => {
  assert.equal(
    resolveMessageTemplate(
      ...
      { user: `Samwise`, duration: `MAX`, admin: `Admin`, playfab: `PLAYER_1`, offenses: `2` }
    ),
    `Samwise|MAX|Admin|PLAYER_1|2`
  )
})
