...
export type UnbanPlayerDependencies = {
	validateUnban: (playerId: number, gameServerId: number, actionId?: number) => Promise<unknown>
...
export async function unbanPlayer(
  ...
  try {
    if (input.gameServerId && input.gameServerId !== gameServerId) throw new Error(`The current server changed. Reopen the action.`)
    await dependencies.validateUnban(input.playerId, gameServerId, input.actionId)
    if (await dependencies.resolveCurrentGameServerId() !== gameServerId) throw new Error(`The current server changed. Reopen the action.`)
  } catch (error) {
    return { ok: false, message: errorMessage(error, `Unban is unavailable.`) }
  }
	...
	try {
        ...
        ...(input.removeOffense ? { removeOffense: true } : {}),
				gameServerId,
			})
		}
	} catch (error) {
...
function defaultDependencies(): UnbanPlayerDependencies {
	return {
		validateUnban: (id, gameServerId, actionId) => actionsApi(`validateUnban`, { id, gameServerId, actionId }),
		resolveCurrentGameServerId,
		coreUnban: playfabId => getCoreApi().unban(playfabId),
		recordRelatedUnban: async ({ playerId, actionId, gameServerId, removeOffense }) => {
			await unwrap(
				await getServerApi().playerActions.unban(playerId, actionId, { gameServerId, removeOffense }),
