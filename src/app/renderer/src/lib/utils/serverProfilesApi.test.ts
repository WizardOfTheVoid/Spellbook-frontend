...
test(`superadmin profile previews request official servers while ordinary profile management keeps its filter`, async () => {
  const originalWindow = Object.getOwnPropertyDescriptor(globalThis, `window`)
  const queries: Record<string, unknown>[] = []
  Object.defineProperty(globalThis, `window`, {
    configurable: true,
    value: { chivServer: { gameServers: { list: async (query: Record<string, unknown>) => {
      queries.push(query)
      return result({ servers: [], meta: { currentPage: 1, pageSize: 100, totalPages: 0, totalResults: 0, hasPrevious: false, hasNext: false } })
    } } } },
  })
  try {
    const { fetchProfileServers } = await import(`./serverProfilesApi`)
    await fetchProfileServers(true)
    await fetchProfileServers(false)
    assert.equal(queries[0].official, undefined)
    assert.equal(queries[1].official, false)
    assert.ok(queries.every(query => query.deleted === `active` && query.includeMainMenu === false))
  } finally {
