...
test(`quick messages resolve server facts and current clocks with their selected prefix`, () => {
  const facts = { ...context, adminsOnline: 0, playersOnline: 12, timezone: `Europe/Oslo`, now: `2026-09-21T12:34:56Z` }
  assert.equal(resolveQuickActionMessage(`[admins_online]/[players_online] [current_time] UTC [current_time_utc]`, facts, `server`),
    `[SB] 0/12 14:34:56 UTC 12:34:56`)
})

test(`quick ServerSay and AdminSay resolve serverSlot from the active server`, () => {
  for (const kind of [`server`, `admin`] as const) {
    const resolved = resolveQuickActionMessage(`[players_online]/[serverSlot]`, { ...context, playersOnline: 43, serverSlot: 40 }, kind)
    assert.equal(resolved, `${kind === `server` ? `[SB]` : `Admin:`} 40/40`)
  }
})

test(`message preview uses the same normalized game text as native submission`, () => {
  assert.equal(resolveQuickActionMessage(`  Caf\u00e9 \u65e5\u672c\u8a9e  `, { admin: `Admin` }, `admin`), `Cafe \u65e5\u672c\u8a9e`)
})
...
test(`unions profile definitions with current server variables`, () => {
  ...
  assert.deepEqual(items.map(item => item.tag), [
    `[admin]`, `[server_name]`, `[serverSlot]`, `[clan_name]`, `[clan_tag]`,
...
test(`does not expose variables that shadow built-ins`, () => {
  ...
  assert.deepEqual(items.map(item => item.tag), [
    `[admin]`, `[server_name]`, `[serverSlot]`, `[clan_name]`, `[clan_tag]`, `[rules_url]`
  ])
})
