...
import { normalizePunishment } from "./playerPunishment"
import {
	getBooleanField,
	getNumberField,
	getStringField,
...
function normalizeDbPlayer(value: unknown): DbPlayerListItem | null {
	...
	return {
		id,
		playfabId,
		isWanted: getBooleanField(value, `isWanted`) ?? false,
		activeBan: `activeBan` in value ? normalizePunishment(value.activeBan) : undefined,
		latestPunishment: normalizePunishment(value.latestPunishment),
		...
		latestNormalizedName:
			...
			getStringField(value, "LatestNormalizedName"),
		matchedAlias:
			getStringField(value, `matchedAlias`) ??
			getStringField(value, `MatchedAlias`),
		lastLogin:
