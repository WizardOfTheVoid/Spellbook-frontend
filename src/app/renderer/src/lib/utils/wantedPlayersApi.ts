...
export function getWantedDisplayMeta(
	...
	return {
		...
		offenseType: wanted.offenseType
			? formatOffenseType(wanted.offenseType)
			: `Offense unavailable`,
		...
		author: getAdminName(wanted.author, wanted.author?.playfabId?.trim() || `Author unavailable`),
...
function parseAuthor(value: unknown): WantedPlayerListItem[`wanted`][`author`] {
	...
	return {
		...
		...(value.displayName === undefined ? {} : { displayName: nullableString(value.displayName) }),
