	...
	return [
		{
			...
			subtitle: formatMetric(dbProfile?.player.profileVisitCount),
			icon: `fa-eye`,
			iconTone: `info`,
		},
		{
...
export function buildNicknameItems(
	nicknames: Pick<PlayerDbProfile[`names`][number], `name` | `lastSeen`>[],
	limit = 200,
	now = new Date(),
): TileItem[] {
	return nicknames.slice(0, limit).map(nickname => {
		const days = Math.max(0, Math.floor((now.getTime() - new Date(nickname.lastSeen).getTime()) / (24 * 60 * 60 * 1000)))
		const hasDate = Number.isFinite(days)
		return {
			title: nickname.name,
			subtitle: hasDate ? `${countLabel(days, `day`, `days`)} since` : `Last seen unknown`,
			tooltip: hasDate ? formatFullDateTime(nickname.lastSeen) : null,
			icon: `fa-user-tag`,
			iconTone: `info`,
		}
	})
