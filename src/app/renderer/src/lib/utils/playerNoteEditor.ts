...
export const createPlayerNoteDocument = (
  ...
  const nodes: JSONContent[] = parsePlayerNoteContent(content).map(segment => {
    ...
    return {
      ...
      attrs: {
        id: segment.id,
        label: getAdminName(user, `User ${segment.id}`),
...
export const filterPlayerNoteSuggestions = (
  ...
  const items = kind === `action`
    ...
    : users.map(user => ({
      kind,
      id: user.id,
      label: getAdminName(user),
      detail: `@${user.username}${user.isActive && !user.bannedAt ? `` : ` · inactive`}`,
      search: `${getAdminName(user)} ${user.username}`.toLocaleLowerCase(),
      priority: score(getAdminName(user), user.username, normalized),
    }))
