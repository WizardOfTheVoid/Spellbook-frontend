...
test(`getServerLabel falls back to the raw name without removing words or spacing`, () => {
  assert.equal(getServerLabel({ name: `[TT] DUEL 1v1`, displayName: `   ` }), `[TT] DUEL 1v1`)
  assert.equal(getServerLabel({ name: `[TT] DUEL 1v1` }), `[TT] DUEL 1v1`)
})
