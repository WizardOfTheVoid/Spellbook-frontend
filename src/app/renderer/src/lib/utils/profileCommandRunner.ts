...
export async function executeProfileAction(
  ...
  const auditFailure = await recordSubmittedActions(
    submittedCommands(resolvedCommands, sentCommands),
    context,
    recordAction,
    unbanApi,
  )
...
function submittedCommands(commands: ResolvedCommand[], sentCount: number): ResolvedCommand[] {
  const submitted: ResolvedCommand[] = []
  for (const command of commands) {
    if (sentCount <= 0) break
    submitted.push(command)
    sentCount -= command.command.commandType === `unban` ? unbanSubmissionCount : 1
  }
  return submitted
}
