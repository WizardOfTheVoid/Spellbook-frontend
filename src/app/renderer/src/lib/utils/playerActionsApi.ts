...
const pageSize = 200
const actionTypes = [`ban`, `kick`, `warn`, `unban`, `mock`] as const
const offenseTypes = [
...
export function parsePlayerAction(value: unknown): PlayerAction {
	try {
		...
		const creditedTeam = value.creditedTeam === undefined || value.creditedTeam === null ? null : record(value.creditedTeam)

		return {
			...(value.creditedTeam === undefined ? {} : {
				creditedTeam: creditedTeam === null ? null : {
					id: positiveInteger(creditedTeam.id),
					name: nullableString(creditedTeam.name),
				},
			}),
			id: positiveInteger(value.id),
      ...(value.removedAt === undefined ? {} : { removedAt: nullableString(value.removedAt) }),
      ...(value.isActiveBan === undefined ? {} : { isActiveBan: boolean(value.isActiveBan) }),
			...
			...(value.isAutomated === undefined ? {} : { isAutomated: boolean(value.isAutomated) }),
			...(value.actionOrigin === undefined ? {} : { actionOrigin: enumValue(value.actionOrigin, [`local`, `request`, `rule`, `wanted`] as const) }),
			...
			author: {
				...
				...(author.displayName === undefined ? {} : { displayName: nullableString(author.displayName) }),
