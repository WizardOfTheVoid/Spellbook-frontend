...
export function teamServerOptions(servers: readonly GameServerRecord[], teamId: number): FormOption[] {
  return servers.map(server => {
    const disabled = server.ownerTeamId !== null && server.ownerTeamId !== teamId
    return {
      value: String(server.id), label: getServerLabel(server), disabled,
      tooltip: disabled ? `Claimed by another team` : undefined
    }
  })
}

export function teamServerClaimsInput(initialIds: ReadonlySet<number>, selectedIds: ReadonlySet<number>): TeamServerClaimsInput {
  return {
    claimServerIds: [...selectedIds].filter(id => !initialIds.has(id)),
    unclaimServerIds: [...initialIds].filter(id => !selectedIds.has(id))
  }
}

export async function updateTeamServers(teamId: number, input: TeamServerClaimsInput): Promise<void> {
  await unwrap(await getServerApi().teams.updateServers(teamId, input), `Could not update claimed servers.`)
  gameServerChanged()
}

export async function getServer(gameServerId: number, query?: { a2s?: boolean }): Promise<GameServerProfile> {
  return unwrap<GameServerProfile>(
    await getServerApi().gameServers.get(gameServerId, query),
    `Game server request failed.`
  )
}

export async function getGameServerPlayers(gameServerId: number): Promise<GameServerPlayers> {
  return unwrap<GameServerPlayers>(
    await getServerApi().gameServers.players(gameServerId),
    `Server players request failed.`
  )
}
