...
import { isPlayerBanActive } from '@spellbook/shared/playerBans'
...
export function isActionBanActive(
	...
	return action.isActiveBan !== false && isPlayerBanActive(action, actions, now)
}

export function actionAuthor(action: Pick<PlayerAction, `author` | `authorId`>): string {
	return getAdminName(action.author, action.author.playfabId?.trim() || `#${action.authorId}`)
}
...
export function actionAuthorWithTeam(action: PlayerAction): string {
	...
	if (action.isAutomated && teamName) return teamName
