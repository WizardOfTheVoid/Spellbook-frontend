import { getAllServers } from './gameServersApi'
import {
  getServerApi,
  ...
  type ServerProfilePresetSummary,
  type ServerProfileSource,
} from "$lib/core";
import { unwrap } from "./apiResult";
import { parsePlayerPresence } from "./playerPresence"
...
export async function fetchPlayerProfile(playfabId: string, visit = false): Promise<PlayerDbProfile> {
  const profile = await unwrap<PlayerDbProfile>(await (visit ? getServerApi().visitPlayerProfileByPlayfab(playfabId) : getServerApi().playerProfileByPlayfab(playfabId)), `Player profile request failed.`)
  return { ...profile, presence: parsePlayerPresence(profile.presence) }
}

export async function refreshPlayerProfile(playfabId: string): Promise<PlayerDbProfile> {
  const profile = await unwrap<PlayerDbProfile>(await getServerApi().refreshPlayerProfileByPlayfab(playfabId), `Player profile refresh failed.`)
  return { ...profile, presence: parsePlayerPresence(profile.presence) }
}

export async function fetchProfileServers(isSuperadmin: boolean) {
  return getAllServers({ official: isSuperadmin ? undefined : false, deleted: `active`, includeMainMenu: false })
}
...
export async function fetchProfilePresets(): Promise<ServerProfilePresetSummary[]> {
  return unwrap<ServerProfilePresetSummary[]>(await getServerApi().serverProfiles.presets(), `Presets request failed.`)
}

export async function fetchProfilePreset(profileId: number): Promise<ServerProfileSource> {
  return unwrap<ServerProfileSource>(await getServerApi().serverProfiles.preset(profileId), `Preset request failed.`)
}
...
export async function setServerProfileEnabled(owner: ProfileOwner, profileId: number, isEnabled: boolean): Promise<boolean> {
  const result = await unwrap<{ isEnabledForUser: boolean }>(
    await getServerApi().serverProfiles.setEnabled(owner, profileId, isEnabled),
    `Profile preference save failed.`
  )
  return result.isEnabledForUser
}
