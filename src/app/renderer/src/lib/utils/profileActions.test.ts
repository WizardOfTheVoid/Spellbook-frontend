...
import { profileActionIcon, profileActionIconColor, visibleServerActionProfiles } from './profileActions'

test(`server action groups omit hidden, disabled, player-only and empty entries`, () => {
  const visible = { ...action([`server_message`]), actionDomain: `server` as const }
  const hidden = { ...visible, showInGameServerActions: false }
  const disabled = { ...visible, isEnabled: false }
  const player = action([`warn`])
  const graphs = [[], [hidden], [disabled], [player], [visible, hidden, disabled, player]]
    .map((actions, id) => ({ profile: { id }, actions })) as ServerProfileGraph[]
  const result = visibleServerActionProfiles(graphs)
  assert.deepEqual(result.map(graph => graph.profile.id), [4])
  assert.deepEqual(result[0].actions, [visible])
  assert.equal(graphs[4].actions.length, 4)
