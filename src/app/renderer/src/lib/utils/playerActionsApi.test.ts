...
test(`preserves credited team details and accepts older action responses`, () => {
	const creditedTeam = { id: 23, name: `Team Name` }
	assert.deepEqual(parsePlayerAction({ ...action(9), creditedTeam }).creditedTeam, creditedTeam)
	assert.equal(parsePlayerAction({ ...action(9), creditedTeam: null }).creditedTeam, null)
	assert.equal(parsePlayerAction(action(9)).creditedTeam, undefined)
	assert.throws(() => parsePlayerAction({ ...action(9), creditedTeam: { id: `23`, name: `Team Name` } }), /Invalid player action data/u)
})
...
test(`preserves author display names and accepts legacy authors without them`, () => {
  const payload = action(9)
  const named = parsePlayerAction({ ...payload, author: { ...payload.author, displayName: `Moderator` } })
  assert.equal(named.author.displayName, `Moderator`)
  assert.equal(parsePlayerAction(payload).author.username, payload.author.username)
})
