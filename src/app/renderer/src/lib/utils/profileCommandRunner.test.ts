...
test(`maps kick and ban commands with the ban duration and configured delays`, async () => {
  const batchCalls: CoreBatchCommand[][] = []
  const core: Pick<ChivCoreApi, "executeBatch"> = {
    executeBatch: async (commands) => {
      batchCalls.push(commands)
...
test(`kick audits ignore legacy profile durations`, async () => {
  const audits: RecordActionByPlayfabInput[] = []
  await executeProfileAction(
    { ...moderationAction, commands: [{ ...moderationAction.commands[0], durationHours: 24 }] },
    { admin, serverName: `Duel Server`, player, gameServer },
    { executeBatch: async () => successResult(1) },
    async input => { audits.push(input) }
  )
  assert.equal(audits[0].duration, null)
})

test(`[offenses] counts punitive actions and excludes unban history`, () => {
  const dbProfile = {
    player: { playfab: { statistics: null } },
    actions: [
      { actionType: `ban` },
      { actionType: `warn` },
      { actionType: `unban` }
...
test(`player message execution carries cached rank, playtime and last login into the shared renderer`, async () => {
  const dbProfile = {
    player: { rank: 25, playtimeHours: 125.5, lastLogin: `2026-09-01T12:00:00Z`, playfab: { statistics: null } },
    actions: []
  } as unknown as PlayerDbProfile
  const action = { ...moderationAction, commands: [{ commandType: `server_message` as const, sortOrder: 0, delayMs: 0,
    message: `[player_rank] [playtime] [last_login_alt]` }] }
  let message = ``
  const result = await executeRecipe(action, { admin, player, dbProfile, gameServer, serverName: gameServer.name }, {
    executeProfileRecipe: async request => {
      assert.equal(request.context.lastLogin, `2026-09-01T12:00:00Z`)
      const prepared = prepareAction(request.recipe, request.target, { ...request.context, now: `2026-09-21T12:00:00Z` })
      message = prepared.commands[0].message
      return { result: successResult(1), prepared }
    }
  })
  assert.equal(result.ok, true)
  assert.equal(message, `25 125.5 20 days ago`)
})
...
test(`local profile messages resolve serverSlot from the active game server`, () => {
  assert.equal(resolveProfileMessage(`[serverSlot]`, {
    admin, serverName: `Duel Server`, gameServer: { ...gameServer, maxPlayers: 40 }
  }), `40`)
})
