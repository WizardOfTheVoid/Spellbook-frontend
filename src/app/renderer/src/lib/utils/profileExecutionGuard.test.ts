...
function installApis(actions: ServerProfileAction[], activeFailure?: CoreCallResult, consoleReady = true): void {
  const snapshot = {
    version: 1,
  ...
  const confirmed = { status: `passed`, message: `Verified` } as const
  const setup = { ...initialConsoleSetupState(), ...(consoleReady ? { enabled: confirmed, binding: confirmed } : {}), commandsBlocked: !consoleReady, commandIssue: consoleReady ? null : `Console setup required` }
  Object.assign(globalThis, {
    window: {
      chivCore: { currentGameSnapshot: async () => snapshot, consoleSetupState: async () => setup,
        onConsoleSetupStateChanged: (listener: (state: typeof setup) => void) => { listener(setup); return () => {} }
      } as unknown as ChivCoreApi,
      chivServer: {
        serverProfiles: { active: async () => activeResult }
      } as ChivServerApi
    }
  })
  consoleSetup.sync(true)
  authState.set({
    loading: false,
    user: {
      id: 3,
...
test(`profile actions cannot execute while console setup is unconfirmed`, async () => {
  installApis([action(12, true)], undefined, false)
  await assert.rejects(profileExecutionGuard(3, `server-9`, false, 12))
})
