...
test(`preserves author display names and accepts legacy authors without them`, () => {
  ...
  const named = parsePlayerNote({ ...payload, author: { ...payload.author, displayName: `Moderator` } })
  assert.equal(named.author.displayName, `Moderator`)
  assert.equal(parsePlayerNote(payload).author.username, payload.author.username)
})


test(`note mentions fall back to Discord usernames when display names are missing`, () => {
  for (const displayName of [``, ` `, null, undefined]) {
    const payload = { ...notePayload(), userReferences: [{ ...userPayload(), displayName }] }
    assert.equal(getAdminName(parsePlayerNote(payload).userReferences[0]), `magic`)
  }
})
