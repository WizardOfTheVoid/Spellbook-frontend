...
test(`Wanted presence preserves permitted details and clears missing or hidden locations`, () => {
	const presence = {
		isOnline: true, observedAt: `2026-09-06T12:00:00.000Z`,
		server: { id: 7, name: `Duel`, durationSeconds: 123, durationObservedAt: `2026-09-06T12:00:00.000Z` }
	}
	assert.deepEqual(parseWantedDetail({ ...wantedDetail(), presence }).presence, presence)
	assert.deepEqual(parseWantedDetail({ ...wantedDetail(), presence: { ...presence, server: null } }).presence, {
		...presence, server: null
	})
	assert.equal(parseWantedDetail(wantedDetail()).presence, null)
})
