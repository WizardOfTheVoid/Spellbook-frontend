import { applyServerMessagePrefix } from '@spellbook/shared/actions/prepareAction'
...
import { RESERVED_VARIABLE_KEYS } from "./serverVariables"

export type QuickActionMessageKind = "admin" | "server"

export type QuickActionMessageContext = Pick<
  MessageTagContext,
  "admin" | "serverName" | "clanName" | "clanTag" | "serverSlot" | "variables" | "tagDefinitions" | "adminsOnline" | "playersOnline" | "timezone" | "now"
>
...
export function quickActionTagItems(
  ...
  const contextValues = new Map<string, string | undefined>([
    ...
    [`serverSlot`, context.serverSlot === null || context.serverSlot === undefined ? undefined : String(context.serverSlot)],
...
export function resolveQuickActionMessage(
  ...
  return normalizeString(`game`, kind ? applyServerMessagePrefix(resolved, kind, context.variables ?? []) : resolved)
}
