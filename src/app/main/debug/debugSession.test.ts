...
test(`disconnect disarms and reconnect automatically rearms without replaying buffered shortcuts`, async () => {
  ...
  await h.session.setEnabled(true)
  await h.session.setArmed(true)
  await h.session.setProducerPaused(`actions`, true)
  h.responses.get = async () => unavailable()
  await h.advance(100)
  assert.equal((await h.session.getState()).armed, false)
  assert.deepEqual(h.producerChanges.at(-1), { producer: `actions`, paused: false })
...
test(`an older producer reply preserves the latest suspension intent`, async () => {
  ...
  await h.session.setEnabled(true)
  const older = h.session.setProducerPaused(`actions`, true)
  await h.session.setProducerPaused(`actions`, false)
  await h.session.setProducerPaused(`actions`, true)
  delayed.resolve()
  await older
  assert.equal(pausedNow, true)
  assert.equal((await h.session.getState()).pausedProducers.actions, true)
})
...
test(`lost pause and settings acknowledgments retain cleanup intent through stop, disable and reconnect`, async () => {
  for (const operation of [`pause`, `settings`] as const) {
    for (const lifecycle of [`stop`, `disable`, `reconnect`] as const) {
      ...
      await h.session.setEnabled(true)
      h.responses.mutation = async () => unavailable()
      await assert.rejects(operation === `pause` ? h.session.queue(`pause`) : h.session.settings({ Queue__UserActionTtlMs: 1000 }))
      h.responses.mutation = undefined
...
test(`a lost mutation reply arriving after stop causes another cleanup attempt`, async () => {
  for (const operation of [`pause`, `settings`] as const) {
    ...
    const delayed = deferred<CoreCallResult>()
    h.responses.mutation = () => delayed.promise
    const mutation = operation === `pause` ? h.session.queue(`pause`) : h.session.settings({ Queue__UserActionTtlMs: 1000 })
    h.responses.mutation = undefined
    await h.session.stop()
    const before = h.requests.length
...
test(`Apply and Reset expose acknowledged effective timings before another poll`, async () => {
  const h = harness()
  await h.session.setEnabled(true)
  h.responses.mutation = async () => envelope({ Queue__UserActionTtlMs: 1234, QUEUE_GATE_NORMAL: 500 })
  await h.session.settings({ Queue__UserActionTtlMs: 1234 })
  assert.deepEqual((await h.session.getState()).core?.settings, { Queue__UserActionTtlMs: 1234, QUEUE_GATE_NORMAL: 500 })
  h.responses.mutation = async () => envelope({ Queue__UserActionTtlMs: 30000, QUEUE_GATE_NORMAL: 500 })
  await h.session.settings({}, false, true)
  assert.equal((await h.session.getState()).core?.settings.Queue__UserActionTtlMs, 30000)
  h.responses.mutation = async () => envelope({ Queue__UserActionTtlMs: `invalid` })
  await assert.rejects(h.session.settings({ Queue__UserActionTtlMs: 1234 }), /Invalid Core debug settings/u)
  assert.equal((await h.session.getState()).core?.settings.Queue__UserActionTtlMs, 30000)
})

test(`a poll started before Apply cannot replace acknowledged timings but later polls remain authoritative`, async () => {
  const h = harness()
  h.core.settings = { Queue__UserActionTtlMs: 30000 }
  await h.session.setEnabled(true)
  const oldSnapshot = structuredClone(h.core)
  const pending = deferred<CoreCallResult>()
  h.responses.get = async () => pending.promise
  await h.advance(100)
  h.responses.mutation = async () => envelope({ Queue__UserActionTtlMs: 1234 })
  await h.session.settings({ Queue__UserActionTtlMs: 1234 })
  pending.resolve(envelope(oldSnapshot))
  await flush()
  assert.equal((await h.session.getState()).core?.settings.Queue__UserActionTtlMs, 1234)
  h.core.settings.Queue__UserActionTtlMs = 5000
  h.responses.get = undefined
  await h.advance(100)
  assert.equal((await h.session.getState()).core?.settings.Queue__UserActionTtlMs, 5000)
})
