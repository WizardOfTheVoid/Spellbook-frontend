...
export class DebugConfiguration {
  ...
  async initialize(): Promise<void> {
    ...
    const layout = validateDebugLayout({ ...createDebugLayout(), ...recordLayout(saved.layout) })
...
function recordLayout(value: unknown): Record<string, unknown> {
  if (!isRecord(value)) throw new RangeError(`Invalid debug layout.`)
  return value
}
