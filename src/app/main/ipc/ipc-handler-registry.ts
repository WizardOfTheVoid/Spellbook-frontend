...
import { rendererUrl } from '../window/rendererRoute'
import { ConsoleSetupIpc } from '../consoleSetup/consoleSetupIpc'
import type { ConsoleSetupService } from '../consoleSetup/consoleSetupService'
import { GameStateIpc } from '../gameState/gameStateIpc'
import type { GameStateService } from '../gameState/gameStateService'
import { AppLinkIpcHandlers } from '../appLinks/appLinkIpcHandlers'
import type { AppLinkInbox } from '../appLinks/appLinkInbox'
...
type IpcHandlerRegistryOptions = {
  ...
  diagnosticLogs: DiagnosticLogService
  consoleSetup: ConsoleSetupService
...
export class IpcHandlerRegistry {
  ...
  register(): void {
    ...
    new DiagnosticLogIpcHandlers(ipcMain, this.options.diagnosticLogs, async event => {
      const parent = BrowserWindow.fromWebContents(event.sender)
      if (!parent) return undefined
      const result = await dialog.showSaveDialog(parent, {
        title: `Save SpellBook logs`,
        defaultPath: `SpellBook-logs-${new Date().toISOString().replace(/[:.]/gu, `-`)}.log`,
        filters: [{ name: `Log files`, extensions: [`log`] }],
      })
      return result.canceled ? undefined : result.filePath
    }, rendererUrl(`/`, is.dev ? process.env.ELECTRON_RENDERER_URL : undefined)).register()
    ...
    new ConsoleSetupIpc(ipcMain, this.options.consoleSetup, overlayWindow).register()
    new GameStateIpc(ipcMain, this.options.gameState, overlayWindow).register()
    this.options.auth.register();
    new AppLinkIpcHandlers(ipcMain, this.options.appLinks, overlayWindow).register()
