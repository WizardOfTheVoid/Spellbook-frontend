...
export class ServerIpcHandlers {
  ...
  private static tickAction(value: unknown): `leaderboard` | `servers` | `cleanup` {
    const action = ServerIpcHandlers.record(value).action

    if (action !== `leaderboard` && action !== `servers` && action !== `cleanup`) {
      throw new Error(`action must be leaderboard, servers or cleanup.`)
    }
