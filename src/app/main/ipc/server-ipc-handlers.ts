...
export class ServerIpcHandlers {
  ...
  private static tickAction(value: unknown): `leaderboard` | `servers` | `cleanup` | `normalizePlayerNames` {
    const action = ServerIpcHandlers.record(value).action

    if (action !== `leaderboard` && action !== `servers` && action !== `cleanup` && action !== `normalizePlayerNames`) {
      throw new Error(`action must be leaderboard, servers, cleanup or normalizePlayerNames.`)
    }
