...
import type { TeamServerClaimsInput } from '@spellbook/shared/teamServers.js'
import type { HttpClient } from '../api/http-client';
import type { ServerPlayersPayload } from '../types';
import { parsePlayerListQuery } from '@spellbook/shared/playerListQuery.js'
import { parsePlayerEnrichmentIds } from '@spellbook/shared/playerEnrichment.js'
...
export class ServerIpcHandlers {
  ...
  register(): void {
    this.ipcMain.handle(`server:players:enrich`, async (_event, payload: unknown) =>
      this.httpClient.postServer(`/players/enrich`, { playfabIds: parsePlayerEnrichmentIds(payload) }))

    this.ipcMain.handle(`server:actions`, async (_event, operation: string, input: Record<string, unknown> = {}) => {
      const id = () => ServerIpcHandlers.requiredId(input.id, `id`)
      switch (operation) {
        case `messageFacts`: return this.httpClient.getServer(`/actions/message-context/${id()}`)
        case `userSettings`: return this.httpClient.getServer(`/users/me/settings`)
        case `saveUserSettings`: return this.httpClient.patchServer(`/users/me/settings`, { settings: input })
        case `requests`: return this.httpClient.getServer(`/actions/requests`)
        case `options`: return this.httpClient.getServer(`/actions/options`)
        case `request`: return this.httpClient.postServer(`/actions/requests`, input)
        case `status`: return this.httpClient.getServer(`/actions/${id()}`)
        case `cancel`: return this.httpClient.postServer(`/actions/${id()}/cancel`, {})
        case `unbanRequest`: return this.httpClient.postServer(`/actions/unban`, input)
        case `offenses`: return this.httpClient.getServer(`/player/${id()}/offenses`)
        case `removeOffense`: {
          const body = ServerIpcHandlers.record(input)
          return this.httpClient.postServer(`/player/${id()}/actions/${ServerIpcHandlers.requiredId(body.actionId, `actionId`)}/remove`, {})
        }
        case `validateUnban`: return this.httpClient.postServer(`/player/${id()}/unban/check`, input)
        case `checkPlayerAction`: return this.httpClient.postServer(`/player-actions/check`, input)
        case `admin`: return this.httpClient.getServer(`/admin/actions`, input as Record<string, string>)
        case `control`: return this.httpClient.postServer(`/admin/actions/control`, input)
        case `tagTypes`: return this.httpClient.getServer(`/definitions/tag-types`)
        case `saveTagType`: return this.httpClient.putServer(`/admin/definitions/tag-types`, input)
        case `definitions`: return this.httpClient.getServer(`/rulesets/definitions`)
        case `previewRule`: return this.httpClient.postServer(`/profiles/${id()}/ruleset/preview`, input)
        case `ruleset`: return this.httpClient.getServer(`/profiles/${id()}/ruleset`)
        case `saveRuleset`: return this.httpClient.putServer(`/profiles/${id()}/ruleset`, input)
        default: throw new Error(`Unknown action operation.`)
      }
    })

    this.ipcMain.handle(`server:admin:notifications:send`, async (_event, payload: unknown) => this.httpClient.postServer(`/admin/notifications`, payload))
    this.ipcMain.handle(`server:admin:discord:guilds`, async () => this.httpClient.getServer(`/discord/broadcasts/guilds`))
    this.ipcMain.handle(`server:admin:discord:queue`, async (_event, payload: unknown) => {
      const beforeId = ServerIpcHandlers.record(payload).beforeId
      return this.httpClient.getServer(`/discord/broadcasts/queue`, beforeId === undefined ? {} : {
        beforeId: ServerIpcHandlers.requiredId(beforeId, `beforeId`)
      })
    })
    this.ipcMain.handle(`server:admin:discord:latest-release`, async () => this.httpClient.getServer(`/discord/broadcasts/latest-release`))
    this.ipcMain.handle(`server:admin:discord:broadcast`, async (_event, payload: unknown) => this.httpClient.postServer(`/discord/broadcasts`, payload))
    this.ipcMain.handle(`server:admin:discord:send`, async (_event, payload: unknown) => this.httpClient.postServer(`/discord/broadcasts/guild`, payload))
    this.ipcMain.handle(`server:admin:discord:broadcast-release`, async (_event, payload: unknown) => this.httpClient.postServer(`/discord/broadcasts/latest-release`, payload))
    ...
    this.ipcMain.handle('server:notifications:list', async (_event, payload: unknown) => {
      const { afterId, beforeId } = ServerIpcHandlers.record(payload)
      if (afterId !== undefined && beforeId !== undefined) throw new Error(`Use either afterId or beforeId.`)
      return this.httpClient.getServer(`/notifications`, {
        ...(afterId === undefined ? {} : { afterId: ServerIpcHandlers.nonNegativeInteger(afterId, `afterId`) }),
        ...(beforeId === undefined ? {} : { beforeId: ServerIpcHandlers.requiredId(beforeId, `beforeId`) })
      })
    })
    this.ipcMain.handle(`server:notifications:delete-all`, async () => this.httpClient.deleteServer(`/notifications`))
    ...
    this.ipcMain.handle(`server:teams:servers:update`, async (_event, payload: unknown) => {
      const source = ServerIpcHandlers.record(payload)
      const teamId = ServerIpcHandlers.requiredId(source.teamId, `teamId`)
      return this.httpClient.patchServer(`/teams/${teamId}/servers`, source.input as TeamServerClaimsInput)
    })
    this.ipcMain.handle(`server:teams:pending-count`, async () => this.httpClient.getServer(`/teams/requests/count`))
    ...
    this.ipcMain.handle(`server:teams:rename`, async (_event, payload: unknown) => {
      const input = ServerIpcHandlers.record(payload)
      const teamId = ServerIpcHandlers.requiredId(input.teamId, `teamId`)
      return this.httpClient.patchServer(`/teams/${teamId}`, { name: input.name })
    })
    this.ipcMain.handle(`server:teams:transfer-ownership`, async (_event, payload: unknown) => {
      const input = ServerIpcHandlers.record(payload)
      const teamId = ServerIpcHandlers.requiredId(input.teamId, `teamId`)
      const userId = ServerIpcHandlers.requiredId(input.userId, `userId`)
      return this.httpClient.patchServer(`/teams/${teamId}/ownership`, { userId })
    })
    ...
    this.ipcMain.handle(`server:gameservers:get`, async (_event, payload: unknown) => {
      ...
      if (source.a2s !== undefined && typeof source.a2s !== `boolean`) throw new Error(`a2s must be a boolean.`)
      return this.httpClient.getServer(`/gameserver/${gameServerId}`, source.a2s === undefined ? undefined : { a2s: String(source.a2s) })
    });
    ...
    this.ipcMain.handle(`server:gameservers:players`, async (_event, payload: unknown) => {
      const gameServerId = ServerIpcHandlers.requiredId(ServerIpcHandlers.record(payload).gameServerId, `gameServerId`)
      return this.httpClient.getServer(`/gameserver/${gameServerId}/players`)
    })
    ...
    this.ipcMain.handle(`server:player:timeline`, async (_event, payload: unknown) => {
      const source = ServerIpcHandlers.record(payload)
      const playerId = ServerIpcHandlers.requiredId(source.playerId, `playerId`)
      const before = ServerIpcHandlers.optionalString(source.before)
      return this.httpClient.getServer(`/player/${playerId}/history`, {
        limit: ServerIpcHandlers.normalizeInteger(source.limit, 30, 1, 100),
        ...(before ? { before } : {})
      })
    })

    this.ipcMain.handle(`server:player:visit-by-playfab`, async (_event, payload: unknown) => {
      const playfabId = ServerIpcHandlers.requiredString(ServerIpcHandlers.record(payload).playfabId, `playfabId`)
      return this.httpClient.postServer(`/players/by-playfab/${encodeURIComponent(playfabId)}/visit`, {})
    })
    ...
    this.ipcMain.handle(`server:profiles:presets`, async () => {
      return this.httpClient.getServer(`/server-profiles/presets`)
    })

    this.ipcMain.handle(`server:profiles:preset`, async (_event, payload: unknown) => {
      const profileId = ServerIpcHandlers.requiredId(ServerIpcHandlers.record(payload).profileId, `profileId`)
      return this.httpClient.getServer(`/server-profiles/presets/${profileId}`)
    })
    ...
    this.ipcMain.handle(`server:profiles:set-enabled`, async (_event, payload: unknown) => {
      const source = ServerIpcHandlers.record(payload)
      const owner = ServerIpcHandlers.owner(source.owner)
      const profileId = ServerIpcHandlers.requiredId(source.profileId, `profileId`)
      if (typeof source.isEnabled !== `boolean`) throw new Error(`isEnabled must be a boolean.`)
      return this.httpClient.patchServer(`${ServerIpcHandlers.profilePath(owner)}/${profileId}/preference`, { isEnabled: source.isEnabled })
    })
  ...
  private static tickAction(value: unknown): `leaderboard` | `servers` | `cleanup` | `normalizePlayerNames` | `enrichEmptyPlayers` {
    const action = ServerIpcHandlers.record(value).action

    if (action !== `leaderboard` && action !== `servers` && action !== `cleanup` && action !== `normalizePlayerNames` && action !== `enrichEmptyPlayers`) {
      throw new Error(`action must be leaderboard, servers, cleanup, normalizePlayerNames or enrichEmptyPlayers.`)
    }
