...
export class OverlayIpcHandlers {
  ...
  register(): void {
    ...
    this.ipcMain.handle(`toast:show`, (event, request: ToastRequest) => this.toastWindow.show(request, () => {
      if (event.sender.isDestroyed()) return
      this.overlayWindow.show()
      event.sender.send(`toast:action`, request.actionId)
    }))
    this.ipcMain.handle(`toast:activate`, (event, actionId: unknown) => this.toastWindow.activate(event.sender.id, actionId))
    this.ipcMain.handle('toast:hide', () => this.toastWindow.hide())
    this.ipcMain.on(`toast:ready`, event => this.toastWindow.ready(event.sender.id))
