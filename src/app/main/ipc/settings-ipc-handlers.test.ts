...
test(`console binds stay local when server preferences are read or saved`, async () => {
  ...
  const settings = { debug: true, audioSfxEnabled: true, audioSfxVolume: 0.5, selectedDisplayId: 7, consoleKey: `Backquote`, overlayKey: `F6`, quickOpenPlayerKey: `F7` }
  ...
  new SettingsIpcHandlers(
    ...
    {
      getServer: async () => ({ ok: true, data: { data: { debug: false, audioSfxEnabled: false, consoleKey: `F6`, overlayKey: `F3`, quickOpenPlayerKey: `F4` } } }),
  ...
  assert.equal(settings.overlayKey, `F6`)
  assert.equal(settings.quickOpenPlayerKey, `F7`)
  await handlers.get(`settings:update`)?.(null, { overlayKey: `F8`, quickOpenPlayerKey: `F9` })
  assert.equal(settings.overlayKey, `F8`)
  assert.equal(settings.quickOpenPlayerKey, `F9`)
  await handlers.get(`settings:update`)?.(null, { consoleKey: `Minus` })
  assert.equal(settings.consoleKey, `Minus`)
  assert.deepEqual(synced, [])
  await handlers.get(`settings:update`)?.(null, { debug: false })
