...
test(`local profile execution loads current definitions before preparing native commands`, async () => {
  ...
  new CoreIpcHandlers(
    ...
    { commands: new ActionBuilder(),
      getServer: async (path: string) => {
        if (path === `/actions/message-context/7`) return { ok: true, data: { ok: true, data: { adminsOnline: 2, playersOnline: 15, timezone: `Europe/Oslo` } } }
  ...
  payload.recipe.commands[1].message = `[admins_online]/[players_online] [current_time]/[current_time_utc]`
  await run({}, { ...payload, context: { ...payload.context, now: `2026-09-21T12:34:56Z` } })
  assert.equal((native[2][1] as { command: string }).command, `Adminsay "2/15 14:34:56/12:34:56"`)
  fail = true
