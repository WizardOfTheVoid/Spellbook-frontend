...
test(`player enrichment forwards validated player IDs to its dedicated endpoint`, async () => {
  const handlers = new Map<string, (_event: unknown, payload?: unknown) => Promise<unknown>>()
  const calls: unknown[] = []
  const ipcMain = { handle: (channel: string, handler: never) => handlers.set(channel, handler) } as unknown as IpcMain
  const result = { ok: true, data: { enrichedPlayerIds: [1] } }
  const client = { postServer: async (path: string, body: unknown) => {
    calls.push({ path, body })
    return result
  } } as unknown as HttpClient
  new ServerIpcHandlers(ipcMain, client).register()
  const handler = handlers.get(`server:players:enrich`)!
  assert.equal(await handler({}, { playfabIds: [` P1 `, `P1`], userId: 99 }), result)
  await assert.rejects(() => handler({}, { playfabIds: [] }), /playfabIds/u)
  assert.deepEqual(calls, [{ path: `/players/enrich`, body: { playfabIds: [`P1`] } }])
})

test(`team server claims forward one explicit batch and reject invalid team IDs`, async () => {
  const handlers = new Map<string, (_event: unknown, payload?: unknown) => Promise<unknown>>()
  const calls: unknown[] = []
  const ipcMain = { handle: (channel: string, handler: never) => handlers.set(channel, handler) } as unknown as IpcMain
  const result = { ok: true, status: 204 }
  const client = { patchServer: async (path: string, body: unknown) => {
    calls.push({ path, body })
    return result
  } } as unknown as HttpClient
  new ServerIpcHandlers(ipcMain, client).register()
  const handler = handlers.get(`server:teams:servers:update`)!
  const input = { claimServerIds: [3, 4], unclaimServerIds: [7] }
  assert.equal(await handler({}, { teamId: 8, input, userId: 999 }), result)
  for (const teamId of [0, -1, 1.5, Number.MAX_SAFE_INTEGER + 1]) {
    await assert.rejects(() => handler({}, { teamId, input }), /teamId/u)
  }
  assert.deepEqual(calls, [{ path: `/teams/8/servers`, body: input }])
})

test(`profile enablement validates its input and cannot forward another user's identity`, async () => {
  const handlers = new Map<string, (_event: unknown, payload?: unknown) => Promise<unknown>>()
  const calls: unknown[] = []
  const ipcMain = { handle: (channel: string, handler: never) => handlers.set(channel, handler) } as unknown as IpcMain
  const client = { patchServer: async (path: string, body: unknown) => { calls.push({ path, body }) } } as unknown as HttpClient
  new ServerIpcHandlers(ipcMain, client).register()
  const handler = handlers.get(`server:profiles:set-enabled`)
  assert.ok(handler)
  await handler({}, { owner: { type: `team`, id: 8 }, profileId: 12, isEnabled: false, userId: 77 })
  await assert.rejects(() => handler({}, { owner: { type: `user`, id: 1 }, profileId: 12, isEnabled: `false` }), /isEnabled/u)
  await assert.rejects(() => handler({}, { owner: { type: `team`, id: 8 }, profileId: 0, isEnabled: true }), /profileId/u)
  assert.deepEqual(calls, [{ path: `/profile-owners/team/8/server-profiles/12/preference`, body: { isEnabled: false } }])
})

test(`server roster IPC validates the server ID and uses the authorized read endpoint`, async () => {
  const handlers = new Map<string, (_event: unknown, payload?: unknown) => Promise<unknown>>()
  const calls: string[] = []
  const ipcMain = { handle: (channel: string, handler: never) => handlers.set(channel, handler) } as unknown as IpcMain
  const client = { getServer: async (path: string) => { calls.push(path) } } as unknown as HttpClient
  new ServerIpcHandlers(ipcMain, client).register()
  const handler = handlers.get(`server:gameservers:players`)!
  await handler({}, { gameServerId: 3 })
  await assert.rejects(() => handler({}, { gameServerId: -1 }), /gameServerId/u)
  assert.deepEqual(calls, [`/gameserver/3/players`])
})

test(`pending request count IPC uses the account endpoint without caller supplied identity`, async () => {
  const handlers = new Map<string, (_event: unknown, payload?: unknown) => Promise<unknown>>()
  const calls: string[] = []
  const ipcMain = { handle: (channel: string, handler: never) => handlers.set(channel, handler) } as unknown as IpcMain
  const client = { getServer: async (path: string) => { calls.push(path) } } as unknown as HttpClient
  new ServerIpcHandlers(ipcMain, client).register()
  await handlers.get(`server:teams:pending-count`)!({}, { userId: 77, isSuperadmin: true })
  assert.deepEqual(calls, [`/teams/requests/count`])
})

test(`Discord queue IPC forwards only a valid cursor to a read endpoint`, async () => {
  const handlers = new Map<string, (_event: unknown, payload?: unknown) => Promise<unknown>>()
  const calls: unknown[] = []
  const ipcMain = { handle: (channel: string, handler: never) => handlers.set(channel, handler) } as unknown as IpcMain
  const client = { getServer: async (path: string, query: unknown) => { calls.push({ path, query }) } } as unknown as HttpClient
  new ServerIpcHandlers(ipcMain, client).register()
  const handler = handlers.get(`server:admin:discord:queue`)!
  await handler({}, { beforeId: 42, ignored: true })
  await assert.rejects(() => handler({}, { beforeId: -1 }), /beforeId/u)
  assert.deepEqual(calls, [{ path: `/discord/broadcasts/queue`, query: { beforeId: 42 } }])
})

test(`Discord admin IPC keeps broadcast, selected guild and release operations distinct`, async () => {
  const handlers = new Map<string, (_event: unknown, payload?: unknown) => unknown>()
  const calls: unknown[] = []
  const result = { ok: true, data: { batchId: `batch`, targets: 1 } }
  const ipcMain = { handle: (channel: string, handler: (_event: unknown, payload?: unknown) => unknown) => handlers.set(channel, handler) } as unknown as IpcMain
  const httpClient = {
    getServer: async (path: string) => { calls.push({ path }) },
  ...
  const body = { title: `News`, description: `Message` }
  const targeted = { ...body, guildId: `123456789012345678` }
  await handlers.get(`server:admin:discord:guilds`)!({})
  await handlers.get(`server:admin:discord:latest-release`)!({})
  assert.equal(await handlers.get(`server:admin:discord:broadcast`)!({}, body), result)
  await handlers.get(`server:admin:discord:send`)!({}, targeted)
  await handlers.get(`server:admin:discord:broadcast-release`)!({}, { version: `1.2.3` })
  assert.deepEqual(calls, [
    { path: `/discord/broadcasts/guilds` }, { path: `/discord/broadcasts/latest-release` },
    { path: `/discord/broadcasts`, body }, { path: `/discord/broadcasts/guild`, body: targeted },
    { path: `/discord/broadcasts/latest-release`, body: { version: `1.2.3` } }
  ])
})
...
test('maps notification IPC controls to the exact server API routes', async () => {
  ...
  await handlers.get(`server:notifications:list`)?.({}, { beforeId: 12 })
  await handlers.get(`server:notifications:delete-all`)?.({})
  ...
  const customNotification = { audience: `teams`, recipientIds: [3, 4], title: `Update`, description: `Tonight's event` }
  await handlers.get(`server:admin:notifications:send`)?.({}, customNotification)

  assert.deepEqual(calls, [
    { method: 'GET', path: '/notifications', query: { afterId: 12 } },
    { method: `GET`, path: `/notifications`, query: { beforeId: 12 } },
    ...
    { method: `POST`, path: `/admin/notifications`, body: customNotification }
  ])
})
...
test(`game server profile IPC forwards the A2S opt-in and rejects nonboolean values`, async () => {
  const handlers = new Map<string, (_event: unknown, payload?: unknown) => Promise<unknown>>()
  const calls: unknown[] = []
  const ipcMain = { handle: (channel: string, handler: never) => handlers.set(channel, handler) } as unknown as IpcMain
  const client = { getServer: async (path: string, query?: unknown) => { calls.push({ path, query }) } } as unknown as HttpClient
  new ServerIpcHandlers(ipcMain, client).register()
  const handler = handlers.get(`server:gameservers:get`)!
  await handler({}, { gameServerId: 7, a2s: true, host: `ignored` })
  await handler({}, { gameServerId: 8 })
  await assert.rejects(() => handler({}, { gameServerId: 7, a2s: `true` }), /a2s/u)
  assert.deepEqual(calls, [
    { path: `/gameserver/7`, query: { a2s: `true` } },
    { path: `/gameserver/8`, query: undefined }
  ])
})
...
test('maps tick action IPC controls to the admin server routes', async () => {
  ...
  await handlers.get(`server:admin:tick-actions:start`)?.({}, { action: `cleanup` })
  await handlers.get(`server:admin:tick-actions:start`)?.({}, { action: `normalizePlayerNames` })
  await handlers.get(`server:admin:tick-actions:start`)?.({}, { action: `enrichEmptyPlayers` })
  ...
  assert.deepEqual(calls, [
    ...
    { method: `POST`, path: `/admin/tick-actions/cleanup/start`, body: {} },
    { method: `POST`, path: `/admin/tick-actions/normalizePlayerNames/start`, body: {} },
    { method: `POST`, path: `/admin/tick-actions/enrichEmptyPlayers/start`, body: {} },
  ...
  const start = handlers.get(`server:admin:tick-actions:start`)!
  await assert.rejects(async () => start({}, { action: `unknown` }), /action must be/)
...
test('maps cached and forced player profile IPC calls separately', async () => {
  ...
  await handlers.get(`server:player:visit-by-playfab`)?.({}, { playfabId: `P/300` })

  assert.deepEqual(calls, [
    { method: 'GET', path: '/players/by-playfab/P300' },
    { method: 'POST', path: '/players/by-playfab/P300/refresh', body: {} },
    { method: `POST`, path: `/players/by-playfab/P%2F300/visit`, body: {} }
  ])
})

test(`player timeline IPC forwards bounded read pages without caller identity`, async () => {
  const handlers = new Map<string, (_event: unknown, payload?: unknown) => Promise<unknown>>()
  const calls: unknown[] = []
  const ipcMain = { handle: (channel: string, handler: never) => handlers.set(channel, handler) } as unknown as IpcMain
  const client = { getServer: async (path: string, query: unknown) => { calls.push({ path, query }) } } as unknown as HttpClient
  new ServerIpcHandlers(ipcMain, client).register()
  const timeline = handlers.get(`server:player:timeline`)!

  await timeline({}, { playerId: 42, limit: 20, before: `cursor`, userId: 99 })
  await timeline({}, { playerId: 42 })
  await timeline({}, { playerId: 42, limit: 101 })
  await assert.rejects(() => timeline({}, { playerId: 0 }), /playerId/u)
  assert.deepEqual(calls, [
    { path: `/player/42/history`, query: { limit: 20, before: `cursor` } },
    { path: `/player/42/history`, query: { limit: 30 } },
    { path: `/player/42/history`, query: { limit: 100 } }
  ])
})
...
test(`team management IPC validates IDs and forwards only the intended fields`, async () => {
  const handlers = new Map<string, (_event: unknown, payload?: unknown) => Promise<unknown>>()
  const calls: unknown[] = []
  const ipcMain = { handle: (channel: string, handler: never) => handlers.set(channel, handler) } as unknown as IpcMain
  const client = { patchServer: async (path: string, body: unknown) => { calls.push({ path, body }) } } as unknown as HttpClient
  new ServerIpcHandlers(ipcMain, client).register()
  const rename = handlers.get(`server:teams:rename`)!
  const transfer = handlers.get(`server:teams:transfer-ownership`)!
  await rename({}, { teamId: 3, name: `Knights`, ownerUserId: 99 })
  await transfer({}, { teamId: 3, userId: 9, isSuperadmin: true })
  await assert.rejects(() => rename({}, { teamId: 0, name: `Knights` }), /teamId/u)
  await assert.rejects(() => transfer({}, { teamId: 3, userId: -1 }), /userId/u)
  assert.deepEqual(calls, [
    { path: `/teams/3`, body: { name: `Knights` } },
    { path: `/teams/3/ownership`, body: { userId: 9 } },
  ])
})
