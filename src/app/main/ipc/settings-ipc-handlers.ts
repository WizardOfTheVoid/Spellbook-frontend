...
import { keybindNames, syncedSettings } from '../../shared/keybinds'
  ...
  register(): void {
    this.ipcMain.handle('settings:get', async () => {
      ...
      if (result.ok && settings) {
        await this.appSettings.updateSettings(syncedSettings(settings))
      }
    ...
    this.ipcMain.handle('settings:update', async (_event, update: unknown) => {
      ...
      if (typeof update === `object` && update !== null && Object.keys(update).some(key => !Object.hasOwn(keybindNames, key))) {
        await this.httpClient.patchServer(`/users/me/settings`, { settings: syncedSettings(snapshot.settings) })
      }
