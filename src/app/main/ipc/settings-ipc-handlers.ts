  ...
  register(): void {
    this.ipcMain.handle('settings:get', async () => {
      ...
      if (result.ok && settings) {
        const { consoleKey: _consoleKey, ...syncedSettings } = settings
        await this.appSettings.updateSettings(syncedSettings)
      }
    ...
    this.ipcMain.handle('settings:update', async (_event, update: unknown) => {
      ...
      if (typeof update === `object` && update !== null && Object.keys(update).some(key => key !== `consoleKey`)) {
        const { consoleKey: _consoleKey, ...syncedSettings } = snapshot.settings
        await this.httpClient.patchServer(`/users/me/settings`, { settings: syncedSettings })
      }
