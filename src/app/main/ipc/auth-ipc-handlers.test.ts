...
test(`moderation identity follows validated sessions and profile changes and clears immediately on logout`, async () => {
  const fixture = createFixture({
    token: `session`,
    getServer: async () => result({ ...user(true), playfabId: `PLAYER_1` }),
    patchServer: async () => result({ ...user(true), playfabId: `PLAYER_2` })
  })
  fixture.auth.register()
  const identities: (string | null)[] = []
  fixture.auth.subscribe(() => { identities.push(fixture.auth.playfabId) })
  await fixture.auth.restoreSession()
  assert.equal(fixture.auth.playfabId, `PLAYER_1`)
  await fixture.handlers.get(`auth:profile`)?.({}, { playfabId: `PLAYER_2` })
  assert.equal(fixture.auth.playfabId, `PLAYER_2`)
  const logout = fixture.handlers.get(`auth:logout`)?.({})
  assert.equal(fixture.auth.playfabId, null)
  await logout
  assert.deepEqual(identities, [`PLAYER_1`, `PLAYER_2`, null])
})

test(`unvalidated or inactive session data cannot supply a moderation identity`, async () => {
  for (const invalid of [null, { playfabId: `PLAYER_2` }, { ...user(true), playfabId: `PLAYER_2`, isActive: false }]) {
    let response = result({ ...user(true), playfabId: `PLAYER_1` })
    const fixture = createFixture({ token: `session`, getServer: async () => response })
    fixture.auth.register()
    await fixture.auth.restoreSession()
    response = result(invalid)
    await fixture.handlers.get(`auth:session`)?.({})
    assert.equal(fixture.auth.playfabId, null)
