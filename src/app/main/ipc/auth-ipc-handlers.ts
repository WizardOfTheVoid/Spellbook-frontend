...
export class AuthIpcHandlers {
  ...
  private async applyValidation(result: CoreCallResult, revision: number): Promise<boolean> {
    ...
    const playfabId = this.dataRecord(result)?.playfabId
    this.playfabId = state?.authenticated && typeof playfabId === `string` ? playfabId.trim() || null : null
