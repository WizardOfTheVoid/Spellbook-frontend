...
export class AuthIpcHandlers {
  ...
  register(): void {
    ...
    this.ipcMain.handle(`auth:logout`, () => {
  ...
  private validatedState(result: CoreCallResult): MainAuthState | null {
    ...
    if (!Number.isInteger(id) || Number(id) < 1 || typeof onboardingComplete !== `boolean`) return null
  ...
  private static staleRequestResult(): CoreCallResult {
    return {
      ...
      error: {
        ...
        message: `A newer authentication intent replaced this request.`
      }
    }
  }
}
