...
function createCoreMessageHandlerFixture(inactiveResult: CoreCallResult | null = null,
  state: GameState = { ...initialGameState, available: true, running: true, connected: true }
): CoreMessageHandlerFixture {
  ...
  new CoreIpcHandlers(
    ...
    createGameChecks(() => state) as GameStateService
  ).register()
...
test('core message forwards an admin payload through Core input', async () => {
  ...
  assert.deepEqual(postCalls, [{
    commands: [{ type: `console`, command: `Adminsay "Hello"`, consoleKey: `NumpadSubtract`, delayMs: 0 }],
    options: { id: `test-message`, author: `user`, priority: `normal` }
  }])
...
test('core message accepts server payloads', async () => {
  ...
  assert.deepEqual(postCalls, [{
    commands: [{ type: `console`, command: `Serversay "Round starts now"`, consoleKey: `NumpadSubtract`, delayMs: 0 }],
    options: { id: `test-message`, author: `user`, priority: `normal` }
  }])
})
...
for (const [name, changes] of Object.entries({
  ...
  menu: { map: `Hastings`, playerCount: 1 },
  disconnected: { connected: false },
  unavailable: { available: false }
})) {
  test(`core message rejects ${name} game state without submitting input`, async () => {
    const { handler, postCalls } = createCoreMessageHandlerFixture(null,
      { ...initialGameState, available: true, running: true, connected: true, ...changes })

    const result = await handler({}, { kind: `server`, message: `Hello` }) as CoreCallResult

    assert.equal(result.ok, false)
    assert.equal(result.error?.code, `NOT_IN_GAME_SERVER`)
    assert.deepEqual(postCalls, [])
  })
}

test(`core message allows a connected server while the game is not focused`, async () => {
  const { handler, postCalls } = createCoreMessageHandlerFixture(null,
    { ...initialGameState, available: true, running: true, connected: true, focused: false })

  const result = await handler({}, { kind: `admin`, message: `Hello` }) as CoreCallResult
...
test(`snapshot IPC reads state and delegates both manual refresh channels to the Main poller`, async () => {
  ...
  new CoreIpcHandlers(
    ...
    {} as HttpClient,
    {} as AppHealthService,
    { refreshNow: async (author: string) => {
      assert.equal(author, `user`)
      refreshCalls += 1
      return expectedResult
    } } as unknown as ListPlayersPoller,
    {
