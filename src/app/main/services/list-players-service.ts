...
import { isMainMenuObservation } from '../../shared/gameState'
import type { ConsoleKeyCode } from '../../shared/consoleKey'
import type { CoreActionAuthor } from '../../shared/coreAction'
...
export type ListPlayersRefresh = Readonly<{
  ...
  observation?: ListPlayersSnapshot
...
export class ListPlayersService {
  ...
  async refresh(mode: GameInputMode, signal?: AbortSignal,
    observeSnapshot?: (snapshot: ListPlayersSnapshot) => boolean, author: CoreActionAuthor = `system`): Promise<ListPlayersRefresh> {
    if (mode === `interactive`) {
    ...
    if (signal?.aborted || observeSnapshot?.(snapshot) === false) return { result, consoleOutput }
    if (isMainMenuObservation(snapshot.serverName, snapshot.players.length)) return { result, consoleOutput, observation: snapshot }

    const ingestResult = await this.httpClient.postServer(`/listplayers`, { ...snapshot, clientId: this.clientId, sequence: ++this.sequence })
    if (!ingestResult.ok) {
      console.warn(ResponseParser.getCallErrorMessage(ingestResult, `Server ListPlayers ingest failed.`))
      return { result: ingestResult, consoleOutput, observation: snapshot }
    }
    ...
    if (!accepted || !Number.isInteger(gameServerId) || gameServerId === null || gameServerId < 1 || !externalId || !timestampUtc) {
      return { result, consoleOutput, observation: snapshot }
    }
