...
export class ListPlayersPoller {
  ...
  async refreshNow(author: CoreActionAuthor = `system`): Promise<CoreCallResult> {
    return (await this.refresh(author)).result
  }

  refresh(author: CoreActionAuthor = `system`): Promise<ListPlayersRefresh> {
    ...
    this.clearTimer()
    return this.run(this.generation, author)
  }
  ...
  private run(generation: number, author: CoreActionAuthor = `system`): Promise<ListPlayersRefresh> {
