    ...
    request = (async () => {
      try {
        ...
        if (decision.kind === `defer`) {
          if (decision.reason === `movement` || decision.reason === `chatting`) this.nextDelayMs = this.activity.recheckMs
          return deferredResult(decision.reason)
        }

        const mode = decision.kind === `hidden-idle` ? `background` : decision.kind
