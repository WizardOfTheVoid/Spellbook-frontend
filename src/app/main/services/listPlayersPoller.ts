  ...
  private run(generation: number): Promise<ListPlayersRefresh> {
    ...
    request = (async () => {
      try {
        ...
        const refreshed = await this.service.refresh(decision.kind, cancellation.signal)
        if (this.active && !this.debugPaused && generation === this.generation && refreshed.candidate) {
