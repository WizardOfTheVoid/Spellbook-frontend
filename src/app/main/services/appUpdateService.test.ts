...
test(`requires major and minor upgrades but allows patch-only and older releases`, async () => {
  const AppUpdateService = await loadAppUpdateService()
  for (const [tag, required] of [[`v2.0.0`, true], [`v1.2.0`, true], [`v1.2.9`, true], [`v1.1.99`, false], [`v1.1.4`, false], [`v1.0.99`, false]] as const) {
    let requests = 0
    const service = new AppUpdateService(`1.1.4`, async () => Response.json({ tag_name: tag }), async () => undefined, async () => { requests += 1 })
    await service.check()
    assert.equal(requests, required ? 1 : 0, tag)
  }
})
