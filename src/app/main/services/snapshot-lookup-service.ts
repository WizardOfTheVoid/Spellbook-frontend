...
/**
 * Runs the Quick Open Player game snapshot: read the bottom of the game window through Core OCR and hand the
...
export class SnapshotLookupService {
  ...
  async run(): Promise<void> {
    if (this.isRunning) {
      log('Quick Open Player', 'ignored, a lookup is already running');
      return;
    }

    this.isRunning = true;
    const startedAtMs = Date.now();
    log('Quick Open Player', 'requesting a game snapshot from Core');
