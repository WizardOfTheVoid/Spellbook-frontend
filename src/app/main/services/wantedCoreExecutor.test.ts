...
for (const mode of [`background`, `sentinel`] as const) test(`${mode} execution sends the correct Core idle policy without overlay guards`, async () => {
  const calls: unknown[] = []
  const executor = createExecutor({
  ...
  assert.deepEqual(await executor.execute(work(), messages(), mode), { ok: true })
  assert.deepEqual(calls, [[`/v2/console/batch`, {
    method: `POST`,
    body: JSON.stringify({
      id: `wanted-ban-id`,
      background: true,
      requireIdle: mode === `background`,
      commands: [
        {
          commandType: `ban`,
          playfabId: `PLAYER_1`,
