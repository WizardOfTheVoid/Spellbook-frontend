...
for (const reason of [`movement`, `chatting`] as const) test(`${reason} defers without Core input and schedules one short recheck`, async () => {
  const scheduler = new FakeScheduler()
  const eligibility = new FakeEligibility({ kind: `defer`, reason })
  let refreshCalls = 0
