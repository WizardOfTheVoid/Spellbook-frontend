...
export class DebugActivityService {
  ...
  constructor(
    ...
    private readonly minimumMovementIdleMs: number,
    private readonly isOverlayFocused: () => boolean,
  ...
  private async refresh(generation: number): Promise<void> {
    ...
    try {
      ...
      if (this.enabled && generation === this.generation) {
        this.display.update(readActivity(result, this.minimumMovementIdleMs, this.isOverlayFocused()))
      }
    } catch {
...
function readActivity(result: CoreCallResult, minimumMovementIdleMs: number, overlayFocused: boolean): GameActivitySnapshot | null {
  ...
  const command = ValueReader.isRecord(data?.lastCommand) ? data.lastCommand : null
  const text = command ? ValueReader.getString(command, `command`) : null
  const timeSinceMs = command ? ValueReader.getNumber(command, `timeSinceMs`) : null
  const lastCommand = text && timeSinceMs !== null ? { command: text, timeSinceMs } : null
  const isAfk = timeSinceMovementMs >= minimumMovementIdleMs
  const focus = ValueReader.isRecord(data?.focus) ? data.focus : null
  const gameFocused = ValueReader.getBoolean(focus, `gameIsFocused`) === true
  return { available, isMoving, isChatting, timeSinceMovementMs, timeSinceChattingMs, chatCooldownRemainingMs, lastCommand, isAfk, gameFocused, overlayFocused }
}
