...
test(`an explicitly requested refresh retains the user author`, async () => {
  const service = createService({
    executeAction: async (_commands, options) => {
      assert.equal(options.author, `user`)
      return ok(null)
    }
  }, null)
  await service.refresh(`interactive`, undefined, undefined, `user`)
})
...
test('interactive refresh retains the foreground guard and typed restore-target path', async () => {
  ...
  assert.deepEqual(calls, [{
    ...
    options: { id: `test-listplayers`, author: `system`, priority: `low` }
  }])
...
test(`a one-player Hastings result is observed locally without backend ingestion`, async () => {
  let ingests = 0
  const service = createService({
    executeAction: async () => ok({ ok: true, data: { serverName: `Hastings`, rawText: `output`,
  ...
  const refreshed = await service.refresh(`background`)
  assert.equal(ingests, 0)
  assert.equal(refreshed.observation?.serverName, `Hastings`)
  assert.equal(refreshed.observation?.players.length, 1)
  assert.equal(refreshed.candidate, undefined)
})
