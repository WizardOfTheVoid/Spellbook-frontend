...
test(`Cursor visibility reaches the app as a boolean with older Core defaulting to false`, () => {
  for (const mouseShowing of [true, false]) {
    assert.equal(readActivity(envelope({ ...status(), mouseShowing }), 60000)?.mouseShowing, mouseShowing)
  }
  assert.equal(readActivity(envelope(status()), 60000)?.mouseShowing, false)
  assert.equal(readActivity(envelope({ ...status(), mouseShowing: `true` }), 60000), null)
})
...
test(`The round-trip check lock does not turn suppressed activity into AFK`, () => {
  const data = status()
  const input = { ...data.input, mouseActive: false, idleMs: 60000, checkLockActive: true }
  const value = readActivity(envelope({ ...data, input }), 60000)!
  assert.equal(value.isAfk, false)
  assert.equal(value.timeSinceMovementMs, 60000)
  assert.equal(readActivity(envelope({ ...data, input: { ...input, checkLockActive: false } }), 60000)?.isAfk, true)
  assert.equal(readActivity(envelope({ ...data, input: { ...input, checkLockActive: `false` } }), 60000), null)
})
