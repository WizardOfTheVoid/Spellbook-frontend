...
export class AppUpdateService {
  constructor(
    ...
    private readonly requireUpdate: () => Promise<void> = async () => undefined,
  ) {}
  ...
  async check(): Promise<string | null> {
    ...
    if (!isNewerVersion(latestVersion, this.currentVersion)) return null
    const [major, minor] = latestVersion.split(`.`)
    const [currentMajor, currentMinor] = this.currentVersion.split(`.`)
    if (major !== currentMajor || minor !== currentMinor) await this.requireUpdate()
