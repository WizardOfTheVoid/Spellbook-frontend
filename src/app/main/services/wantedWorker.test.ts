...
for (const kind of [`hidden-idle`, `sentinel`] as const) test(`${kind} eligibility executes without overlay guards`, async () => {
  const modes: WantedExecutionMode[] = []
  let guardChecks = 0
  const harness = createHarness({
    listWork: async () => page([work()]),
  ...
  assert.deepEqual(modes, [kind === `hidden-idle` ? `background` : `sentinel`])
  assert.equal(guardChecks, 0)
})

for (const reason of [`movement`, `chatting`] as const) test(`${reason} defers before claim and schedules one short recheck`, async () => {
  const harness = createHarness({
    listWork: async () => page([work()]),
    eligibility: async () => ({ kind: `defer`, reason })
  })
  harness.snapshots.replace(snapshot())
