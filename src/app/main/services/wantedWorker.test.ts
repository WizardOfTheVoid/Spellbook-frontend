...
test(`Debug suspension prevents Wanted claims and preserves lifecycle on resume`, async () => {
  let workCalls = 0
  const harness = createHarness({ listWork: async () => { workCalls += 1; return page() } })
  harness.snapshots.replace(snapshot())
  await harness.worker.setDebugPaused(true)
  harness.worker.start()
  await assert.rejects(harness.worker.runNow(), /paused/u)
  harness.sentinel.set(true)
  assert.equal(workCalls, 0)
  assert.equal(harness.scheduler.size, 0)
  await harness.worker.setDebugPaused(false)
...
test(`Resuming Debug waits for old Wanted work before scheduling another pulse`, async () => {
  const pending = deferred<WantedWorkPage>()
  let calls = 0
  const harness = createHarness({ listWork: async () => { calls += 1; return calls === 1 ? pending.promise : page() } })
  harness.snapshots.replace(snapshot())
  harness.worker.start()
  const pausing = harness.worker.setDebugPaused(true)
  const resuming = harness.worker.setDebugPaused(false)
  assert.equal(harness.scheduler.size, 0)
...
test(`no snapshot makes no Server call and schedules the next high-priority pulse`, async () => {
  let workCalls = 0
...
for (const kind of [`background`, `sentinel`] as const) test(`${kind} eligibility executes without overlay guards`, async () => {
  const modes: WantedExecutionMode[] = []
  let guardChecks = 0
  const harness = createHarness({
    listWork: async () => page([work()]),
    eligibility: async () => ({ kind }),
    guard: () => {
      guardChecks += 1
      return null
    },
...
test(`re-resolves exact post-claim player text before Core and completion`, async () => {
  ...
  const harness = createHarness({
    ...
    resolve: (_item, _messageContext, name) => {
      ...
      return {
        ...
        mockServersay: `[SB] mock for ${name}`
      }
    },
  ...
  assert.deepEqual(executed, [{
    ...
    mockServersay: `[SB] mock for  \tFresh \u73a9\u5bb6 "Name"\t `
  }])
...
for (const code of [`ACTION_EXPIRED`, `INPUT_FAILED`, `ACTION_CANCELLED`]) test(`submitted ban is recorded instead of released when its announcement ends with ${code}`, async t => {
  const warnings: unknown[][] = []
  t.mock.method(console, `warn`, (...values: unknown[]) => warnings.push(values))
  const events: unknown[] = []
  let recorded = false
  const harness = createHarness({
    listWork: async () => page(recorded ? [] : [work({ cycleRevision: 4 }), work({ wantedId: 8, sourceActionId: 42 })]),
    execute: async () => {
      events.push(`execute`)
      return { ok: false, sentCommands: 1, failure: { code, message: `Announcement was not submitted.` } }
    },
    recordAttempt: async (itemClaim, item, reason) => {
      events.push([`attempt`, itemClaim.id, item.cycleRevision, item.attemptNumber, reason])
      recorded = true
    },
    refreshNow: async () => {
      events.push(`refresh`)
  ...
  assert.deepEqual(events, [`execute`, [`attempt`, claim().id, 4, 1, `[SB] reason 7`], `refresh`])
  assert.equal(harness.calls.claim, 1)
  assert.equal(warnings.length, 1)
  assert.deepEqual(warnings[0]?.[1], {
    wantedId: 7, sourceActionId: 41, sentCommands: 1,
    failure: { code, message: `Announcement was not submitted.` }
  })
})

test(`partial unban still releases the failed claim without recording a ban attempt`, async () => {
  const events: string[] = []
  const harness = createHarness({
    listWork: async () => page([work({ actionType: `unban` })]),
    execute: async () => ({ ok: false, sentCommands: 1, failure: { code: `ACTION_EXPIRED`, message: `Unban incomplete.` } }),
    fail: async () => { events.push(`fail`) },
    recordAttempt: async () => { events.push(`unexpected attempt`) },
    complete: async () => { events.push(`unexpected complete`) }
  })
  harness.snapshots.replace(snapshot())
  harness.worker.start()
...
test(`partial-ban attempt acknowledgement loss never turns into a claim failure`, async t => {
  t.mock.method(console, `warn`, () => undefined)
  const events: string[] = []
  const harness = createHarness({
    listWork: async () => page([work()]),
    execute: async () => ({ ok: false, sentCommands: 1, failure: { code: `ACTION_EXPIRED`, message: `Announcement expired.` } }),
    recordAttempt: async () => { events.push(`attempt`); throw new Error(`Acknowledgement lost`) },
    fail: async () => { events.push(`unexpected fail`) },
    refreshNow: async () => { events.push(`unexpected refresh`); return { ok: true, status: 200, statusText: `OK`, data: null } }
  })
  harness.snapshots.replace(snapshot())
  harness.worker.start()
  await harness.worker.runNow()
...
test(`stop after ban submission still waits for its durable attempt before settling`, async t => {
  t.mock.method(console, `warn`, () => undefined)
  ...
  const harness = createHarness({
    listWork: async () => page([work()]),
    execute: async () => { events.push(`execute`); return core.promise },
    recordAttempt: async () => { events.push(`attempt`); return attempt.promise },
    fail: async () => { events.push(`unexpected fail`) }
  })
  harness.snapshots.replace(snapshot())
  harness.worker.start()
  await until(() => events.includes(`execute`))

  const stopping = harness.worker.stop()
  core.resolve({ ok: false, sentCommands: 1, failure: { code: `ACTION_CANCELLED`, message: `Announcement cancelled.` } })
  await until(() => events.includes(`attempt`))
  assert.equal(await settles(stopping), false)
  attempt.resolve()
  await stopping
  assert.deepEqual(events, [`execute`, `attempt`])
})
...
test(`stop during Core awaits its result and completion settlement`, async () => {
  ...
  const harness = createHarness({
    listWork: async () => page([work()]),
    execute: async (_work, _messages, _mode, cancellation) => {
      signal = cancellation
      events.push(`core`)
      return core.promise
    },
...
function createHarness(overrides: Overrides = {}) {
  ...
  const dependencies: WantedWorkerDependencies = {
    ...
    cadence: resolvePulseConfig({ PULSE_LOW_MS: `20000`, PULSE_NORMAL_MS: `11000`, PULSE_HIGH_MS: `6000`, PULSE_SENTINEL_MS: `3000` }),
    scheduler
  }
