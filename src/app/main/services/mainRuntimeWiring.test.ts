...
test(`startup checks updates before restoring a session or accepting a login ticket`, async () => {
  for (const initialAuthUrl of [undefined, `spellbook://auth?ticket=login`]) {
    const events: string[] = []
    await initializeWindowBeforeAuth({
      registerIpc: () => undefined,
      createWindow: () => { events.push(`window`) },
      checkUpdates: async () => { events.push(`update`) },
      restoreSession: async () => { events.push(`restore`) },
      initialAuthUrl,
      acceptProtocolUrl: async () => { events.push(`ticket`); return true },
      handleSessionFailure: error => { throw error }
    })
    assert.deepEqual(events, [`window`, `update`, initialAuthUrl ? `ticket` : `restore`])
  }
})

test(`a failed release lookup does not prevent session restoration`, async () => {
  let restored = false
  await initializeWindowBeforeAuth({
    registerIpc: () => undefined,
