...
export function readActivity(result: CoreCallResult, minimumMovementIdleMs: number): GameActivitySnapshot | null {
  ...
  const data = {
    ...raw,
    mouseShowing: raw.mouseShowing === undefined ? false : raw.mouseShowing,
  ...
  return {
    ...
    isAfk: inGame && input.available && !input.checkLockActive &&
...
function isCoreStatus(value: unknown): value is CoreStatusSnapshot {
  ...
  if (typeof value.mouseShowing !== `boolean`) return false
  ...
  if (!hasBooleans(runtime, [`enabled`, `gameRunning`, `appRunning`]) ||
    (input.checkLockActive !== undefined && typeof input.checkLockActive !== `boolean`) ||
