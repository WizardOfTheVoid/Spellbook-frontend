...
export class WantedCoreExecutor {
  ...
  async execute(
    ...
    if (mode !== `interactive`) return classify(await this.background(path, payload, mode === `background`))
  ...
  private background(path: string, payload: JsonRecord, requireIdle: boolean): Promise<CoreCallResult> {
    const { id, ...rest } = payload
    return this.http.callCore(path, {
      method: `POST`,
      body: JSON.stringify({ id, background: true, requireIdle, ...rest })
    })
  }
}
