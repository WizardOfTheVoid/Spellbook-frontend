...
test(`prepares all structured game text once before Core without changing raw commands or IDs`, async () => {
  ...
  await withFetchMock(async input => {
    ...
    const client = new CoreHttpClient(`http://127.0.0.1:48225`, `test`)
    const text = `  M\u2206GIC     \u2665\t\tHi  `
    for (const route of [`message`, `warn`, `ban`, `kick`]) {
      const field = route === `ban` || route === `kick` ? `reason` : `message`
      await client.call(`/v2/console/${route}`, {
        method: `POST`, body: JSON.stringify({ playfabId: `Ab_CD`, [field]: text, background: true })
      })
    }
    await client.call(`/v2/console/batch`, {
      method: `POST`, body: JSON.stringify({ commands: [
        { commandType: `ban`, message: text, playfabId: `Ab_CD`, hours: 10, delayMs: 0 },
        { commandType: `server_message`, message: `Hi   \t\tThere`, delayMs: 10 },
        { commandType: `unban`, playfabId: `Ab_CD`, delayMs: 0 }
      ] })
    })
    await client.call(`/v2/console/command`, {
      method: `POST`, body: JSON.stringify({ command: text })
    })
    assert.equal(bodies.length, 6)
    for (const [index, field] of [`message`, `message`, `reason`, `reason`].entries()) {
      assert.deepEqual(bodies[index], { playfabId: `Ab_CD`, [field]: `MAGIC  heart  Hi`, background: true })
    }
    assert.deepEqual(bodies[4], { commands: [
      { commandType: `ban`, message: `MAGIC  heart  Hi`, playfabId: `Ab_CD`, hours: 10, delayMs: 0 },
      { commandType: `server_message`, message: `Hi    There`, delayMs: 10 },
      { commandType: `unban`, playfabId: `Ab_CD`, delayMs: 0 }
    ] })
    assert.deepEqual(bodies[5], { command: text })
  })
})
