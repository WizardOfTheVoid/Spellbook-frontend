...
export class HttpClient {
  ...
  constructor(
    ...
    this.commands = new ActionBuilder(options.getConsoleKey, options.getOwnPlayfabId)
  ...
  async callCore(path: string, init?: RequestInit): Promise<CoreCallResult> {
    ...
    if (action && typeof init?.body === `string`) {
      const request = ResponseParser.parseText(init.body) as { commands?: CoreCommand[] } | null
      if (Array.isArray(request?.commands)) this.commands.validateTargets(request.commands)
    }
