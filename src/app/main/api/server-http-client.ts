...
export class ServerHttpClient {
  ...
  get authenticatedEpoch(): number | null {
    return this.authToken.trim().length > 0 ? this.authEpoch : null
  }
  ...
  async post(path: string, body: unknown, signal?: AbortSignal): Promise<CoreCallResult> {
  ...
  private async send(method: MutationMethod, path: string, body?: unknown, signal?: AbortSignal): Promise<CoreCallResult> {
