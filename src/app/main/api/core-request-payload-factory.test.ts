...
test('serializes a four-byte native window handle', () => {
  ...
  const result = factory.appTarget()
...
test('serializes an eight-byte native window handle without losing precision', () => {
  ...
  assert.equal(
    factory.appTarget().windowHandle,
    '0xFEDCBA9876543210'
  )
})
...
test('fetches the overlay window for every payload', () => {
  ...
  assert.equal(factory.appTarget().windowHandle, '0x000000000012ABCD')
  assert.equal(factory.appTarget().windowHandle, '0xFEDCBA9876543210')
  assert.equal(providerCalls, 2)
})

test('rejects zero-valued native window handles', () => {
  assert.throws(() => createFactory(Buffer.alloc(4)).appTarget())
  assert.throws(() => createFactory(Buffer.alloc(8)).appTarget())
})

test('rejects native window handles that are not four or eight bytes', () => {
  assert.throws(() => createFactory(Buffer.alloc(3)).appTarget())
  assert.throws(() => createFactory(Buffer.alloc(9)).appTarget())
})
