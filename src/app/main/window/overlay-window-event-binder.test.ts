...
test(`in-app F12 toggles DevTools unless recording a keybind`, () => {
  let recording = false
  let toggles = 0
  let prevented = 0
  const fixture = createWindowFixture()
  new OverlayWindowEventBinder({
    hide: () => undefined,
    toggleDevTools: () => { toggles += 1 },
    clearWindow: () => undefined,
    requestFocusRefresh: () => undefined,
    isModalOpen: () => false,
    isKeybindRecording: () => recording,
    clearModalState: () => undefined
  }).bind(fixture.window)
  const event = { preventDefault: () => { prevented += 1 } }
  fixture.beforeInput(event, { type: `keyDown`, key: `F12` })
  fixture.beforeInput(event, { type: `keyUp`, key: `F12` })
  recording = true
  fixture.beforeInput(event, { type: `keyDown`, key: `F12` })
...
test(`mouse browser commands send one renderer navigation and ignore unrelated commands`, () => {
  const fixture = createWindowFixture()
  let appCommand: ((event: unknown, command: string) => void) | undefined
  const sent: unknown[][] = []
  fixture.window.on = ((event: string, callback: typeof appCommand) => {
    if (event === `app-command`) appCommand = callback
    return fixture.window
  }) as typeof fixture.window.on
  fixture.window.webContents.send = (...args) => { sent.push(args) }
  new OverlayWindowEventBinder({
    hide: () => {}, toggleDevTools: () => {}, clearWindow: () => {},
    requestFocusRefresh: () => {}, isModalOpen: () => true, clearModalState: () => {},
  }).bind(fixture.window)
  assert.ok(appCommand)
  appCommand({}, `browser-backward`)
  appCommand({}, `browser-forward`)
  appCommand({}, `volume-up`)
  assert.deepEqual(sent, [[`overlay:history`, `back`], [`overlay:history`, `forward`]])
})
