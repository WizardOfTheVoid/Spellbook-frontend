...
export class DebugWindowController {
  ...
  private session: DebugSessionSnapshot | null = null
  ...
  updateSession(session: DebugSessionSnapshot): void {
    this.session = session
    this.getCurrent()?.webContents.send(`debug:state`, session)
  }
  ...
  private getOrCreate(): BrowserWindow {
    ...
    window.webContents.on(`did-finish-load`, () => {
      ...
      if (this.session) this.updateSession(this.session)
    })
