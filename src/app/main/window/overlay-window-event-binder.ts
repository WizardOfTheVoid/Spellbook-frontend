...
export class OverlayWindowEventBinder {
  ...
  bind(window: BrowserWindow): void {
    ...
    window.on(`app-command`, (_event, command) => {
      if (!window.isVisible() || window.webContents.isDestroyed()) return
      if (command === `browser-backward` || command === `browser-forward`) {
        window.webContents.send(`overlay:history`, command === `browser-backward` ? `back` : `forward`)
      }
    })

    window.webContents.on('before-input-event', (event, input) => {
      if (this.options.isKeybindRecording?.()) return
