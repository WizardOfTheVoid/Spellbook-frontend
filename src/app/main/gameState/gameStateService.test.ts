...
test(`monitoring keeps detecting game exit without renderer polling`, async context => {
  context.mock.timers.enable({ apis: [`setInterval`] })
  let running = true
  let calls = 0
  const snapshots = new CurrentGameSnapshotStore()
  const game = new GameStateService({ callCore: async path => {
    if (path === `/v2/meta/get`) calls++
    return meta(running)
  } }, snapshots)
  game.start()
  await settle()
...
test(`a delayed stopped-game metadata response cannot clear a newer server observation`, async () => {
  ...
  const snapshots = new CurrentGameSnapshotStore()
  const game = new GameStateService({ callCore: async path => path !== `/v2/meta/get` || ++calls === 1 ? meta(true) : waiting }, snapshots)
  await game.refresh()
...
test(`metadata refresh sends the main-menu helper result to Core`, async () => {
  const states: unknown[] = []
  const snapshots = new CurrentGameSnapshotStore()
  const game = new GameStateService({ callCore: async (path, init) => {
    if (path === `/v3/runtime/game`) states.push(JSON.parse(String(init?.body)))
    return meta(true)
  } }, snapshots)
  await game.refresh()
  game.observe(observation(`Hastings`, 1), snapshots.invalidationVersion)
  await game.refresh()
  game.observe(observation(`Hastings`, 2), snapshots.invalidationVersion)
  await game.refresh()
  assert.deepEqual(states, [{ isMainMenu: false }, { isMainMenu: true }, { isMainMenu: false }])
})
