...
export class GameStateService {
  ...
  private async readMeta(): Promise<CoreCallResult> {
    ...
    try {
      await this.http.callCore(`/v3/runtime/game`, { method: `POST`,
        body: JSON.stringify({ isMainMenu: this.isMainMenu() }), signal: AbortSignal.timeout(5_000) })
    } catch { /* The next metadata refresh retries the runtime state. */ }
    return result
  }
