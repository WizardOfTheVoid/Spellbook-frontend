...
const verifiedBinding = { status: `passed`, message: `In-game console key verified` } as const
...
export class ConsoleSetupService {
  ...
  constructor(
    ...
    private readonly verification = new ConsoleVerificationStore()
  ) {
    this.state = initialConsoleSetupState(getConsoleKey() ?? `NumpadSubtract`)
  }

  async loadVerification(): Promise<void> {
    try { await this.verification.load() }
    catch (error) { console.warn(`Console verification could not be loaded. Starting unchecked.`, error) }
    this.settingsChanged()
    this.publish({ binding: this.savedBinding() })
  }

  flushVerification(): Promise<void> { return this.verification.flush() }
  ...
  stop(): void {
    ...
    this.publish({ ...initialConsoleSetupState(this.key()), binding: this.savedBinding() })
  }
  ...
  settingsChanged(): void {
    ...
    this.publish({ consoleKey: this.key(), binding: this.savedBinding() })
  }
  ...
  observeListPlayers(result: CoreCallResult, consoleKey: ConsoleKeyCode): void {
    ...
    if (this.state.binding.status === `checking` || this.bindingFailed) return
  ...
  private async checkBinding(retry: boolean): Promise<boolean> {
    ...
    const hadPass = this.bindingVerified
    ...
    try {
      ...
      if (!enabled) {
        this.publish({ binding: hadPass ? verifiedBinding : { status: `unavailable`, message: `Enable the in-game console before checking its key.` } })
        return false
      }
      ...
      if (revision !== this.bindingRevision) throw new Error(`Console key changed. Check the new key.`)
      this.publish({ binding: { status: `checking`, message: `Checking console key…` } })
      if (refreshed.consoleOutput?.key === this.key()) {
        const result = refreshed.consoleOutput.result
        const data = ValueReader.getEnvelopeData(result)
        const snapshot = ListPlayersSnapshotParser.extract(result)
        if (result.ok && data?.status === `completed` && data.sent === true && snapshot?.rawText.trim()) {
          this.passBinding()
        ...
        if (errorCode(result) === `CLIPBOARD_TIMEOUT` && Number(data?.sentCommands) > 0) {
          this.verification.revoke(this.key())
      ...
      if (!signal.aborted && revision === this.bindingRevision) {
        this.publish({ binding: hadPass ? verifiedBinding : { status: `unavailable`, message: message(error) } })
      }
      throw error
    } finally {
      this.retryAvailable = false
  ...
  private savedBinding() { return this.verification.has(this.key()) ? verifiedBinding : uncheckedConsole }

  private passBinding(): void {
    this.verification.markPassed(this.key())
    this.publish({ binding: verifiedBinding })
  }
