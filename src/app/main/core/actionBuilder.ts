...
export class ActionBuilder {
  constructor(private readonly getConsoleKey: () => ConsoleKeyCode | null = () => null) {}
  ...
  batch(commands: CoreBatchCommand[]): CoreConsoleCommand[] {
    ...
    return commands.flatMap(command => {
      ...
      switch (command.commandType) {
        ...
        default: throw new RangeError(`Command type must be server_message, warn, kick, ban, or unban.`)
      }
    })
  }
...
export function buildAction(commands: CoreCommand[], app: CoreAppTarget, options: CoreActionOptions): CoreAction {
  ...
  return { id: options.id ?? randomUUID(), key: createHash(`sha256`).update(stableJson(payload)).digest(`hex`), ...payload }
}
