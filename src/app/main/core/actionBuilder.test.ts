...
test(`priority matrix preserves user bans and Wanted source policy in both Sentinel modes`, () => {
  const builder = new ActionBuilder()
  for (const sentinel of [false, true]) {
    ...
    assert.equal(actionPriority(`user`, builder.ban(`PLAYER_1`, 1, `reason`), sentinel), `high`)
    ...
    assert.equal(actionPriority(`wanted`, builder.ban(`PLAYER_1`, 1, `reason`), sentinel), `high`)
    assert.equal(actionPriority(`wanted`, builder.unban(`PLAYER_1`), sentinel), `high`)
    assert.equal(actionPriority(`wanted`, builder.message(`admin`, `mock`), sentinel), `high`)
    assert.equal(actionPriority(`user`, builder.raw(`listplayers`), sentinel), sentinel ? `high` : `low`)
  }
})
