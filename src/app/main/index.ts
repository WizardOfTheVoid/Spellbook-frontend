...
import { DebugActivityService } from './services/debugActivityService'
...
const gameCommandEligibility = new GameCommandEligibility(overlayWindowController, httpClient, sentinelService)
const debugWindow = new DebugWindowController(
	new DebugWindowFactory(__dirname, () => appSettingsService.getSelectedDisplay())
)
const debugActivity = new DebugActivityService(
	httpClient, debugWindow, antiAfkConfig.minimumMovementIdleMs, () => overlayWindowController.isFocused()
)
appSettingsService.setChangeHandler(settings => debugActivity.setEnabled(settings.debug))
...
const authIpcHandlers = new AuthIpcHandlers(ipcMain, httpClient, authSessions, overlayWindowController, undefined, userId => userActivity.update(userId))
...
const quitGuard = new RuntimeQuitGuard(
	async () => {
		...
		debugActivity.setEnabled(false)
		try {
	...
	app.whenReady().then(async () => {
		...
		void startApplication({
			...
			startMonitor: () => {
				...
				focusMonitor.start()
				debugActivity.setEnabled(appSettingsService.getSettings().debug)
			},
