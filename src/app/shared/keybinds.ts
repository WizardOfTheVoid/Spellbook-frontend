...
export function syncedSettings<T extends object>(settings: T): Omit<T, KeybindName> {
  const { consoleKey, overlayKey, quickOpenPlayerKey, ...synced } = settings as T & Partial<KeybindSettings>
  return synced
}
