...
export type DebugPanelName = `tests` | `queue` | `state` | `events` | `ruleset`
...
export type DebugProducer = `listPlayers` | `actions` | `antiAfk`
...
export type DebugSessionSnapshot = {
  ruleset?: { snapshot: RulesetDebugSnapshot | null, receivedAt: number, error: string | null }
  hudVisible?: boolean
