...
public sealed class DebugTimingSettingsTests
{
    ...
    public void authorLifetimesRemainIndependentAcrossOverridesAndReset()
    {
        ...
        Assert.Equal(10000, settings.snapshot()["Queue__SystemActionTtlMs"]);
        settings.update(new Dictionary<string, double> { ["Queue__UserActionTtlMs"] = 45000 });
        settings.update(new Dictionary<string, double> { ["Queue__SystemActionTtlMs"] = 15000, ["QUEUE_GATE_NORMAL"] = 50 });
        ...
        Assert.Equal(10000, settings.CurrentValue.Queue.SystemActionTtlMs);
    }
