...
public sealed class GameInputActivityGateTests
{
    ...
    [InlineData(0x55)]
    [InlineData(0x0D)]
    public void ChattingBlocksBackgroundCommandsAfterMovementStops(int key)
    {
        var time = new ManualTimeProvider();
        var tracker = new MovementActivityTracker(time);
        tracker.MarkAvailable();
        tracker.RecordKeyDown(key, isInjected: false, gameIsForeground: true);
        tracker.RecordKeyUp(key, isInjected: false, gameIsForeground: true);
        time.Advance(TimeSpan.FromSeconds(2));
        Assert.False(tracker.GetSnapshot().IsMoving);
        var gate = new GameInputActivityGate(new FakeGameLocator(FoundGame()),
            new FakeWindowApi { ForegroundWindowHandle = GameWindow }, tracker);

        Assert.NotNull(gate.Check("chat", requireIdle: true));
        Assert.Null(gate.Check("manual", requireIdle: false));
        time.Advance(TimeSpan.FromSeconds(23));
        Assert.Null(gate.Check("expired", requireIdle: true));
    }
