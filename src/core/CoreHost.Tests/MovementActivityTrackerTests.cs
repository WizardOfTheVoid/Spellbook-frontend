...
public sealed class MovementActivityTrackerTests
{
    ...
    [Fact]
    public void ChatTimerRestartsAndExpiresIndependentlyOfMovement()
    {
        var time = new ManualTimeProvider();
        var tracker = new MovementActivityTracker(time, chatCooldownMs: 25000);
        tracker.MarkAvailable();
        Assert.False(tracker.isChatting());
        Assert.Null(tracker.timeSinceChatting());
        tracker.RecordKeyDown(0x59, false, true);
        tracker.RecordKeyUp(0x59, false, true);
        time.Advance(TimeSpan.FromSeconds(20));
        Assert.True(tracker.isChatting());
        Assert.Equal(20000L, tracker.timeSinceChatting());
        Assert.False(tracker.GetSnapshot().IsMoving);
        Assert.Equal(5000L, tracker.GetSnapshot().ChatCooldownRemainingMs);
        tracker.RecordKeyDown(0x0D, false, true);
        tracker.RecordKeyUp(0x0D, false, true);
        time.Advance(TimeSpan.FromMilliseconds(24999));
        Assert.True(tracker.isChatting());
        Assert.Equal(1L, tracker.GetSnapshot().ChatCooldownRemainingMs);
        time.Advance(TimeSpan.FromMilliseconds(1));
        Assert.False(tracker.isChatting());
        Assert.Equal(0L, tracker.GetSnapshot().ChatCooldownRemainingMs);
    }

    [Theory]
    [InlineData(0x59, true, true)]
    [InlineData(0x55, false, false)]
    [InlineData(0x0D, true, true)]
    [InlineData(0x57, false, true)]
    public void NonChatActivityDoesNotRestartOrClearChatCooldown(int key, bool injected, bool foreground)
    {
        var time = new ManualTimeProvider();
        var tracker = new MovementActivityTracker(time);
        tracker.MarkAvailable();
        ...
        tracker.RecordKeyUp(0x59, false, true);
        time.Advance(TimeSpan.FromSeconds(20));
        tracker.RecordKeyDown(key, injected, foreground);
        tracker.RecordMouseButtonDown(false, true);
        Assert.True(tracker.isChatting());
        Assert.Equal(20000L, tracker.timeSinceChatting());
