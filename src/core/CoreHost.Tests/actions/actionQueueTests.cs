...
public sealed class ActionQueueTests
{
    ...
    public void customLifetimeIsClampedToConfiguredBudget(int ttl, int expected)
    {
        var json = JsonSerializer.Serialize(action("ttl"));
        var request = JsonSerializer.Deserialize<CoreActionRequest>(json[..^1] + $",\"TtlMs\":{ttl}}}")!;
        var queue = new ActionQueue();
        _ = queue.enqueue(request, 100, 10000, 999, TestContext.Current.CancellationToken);
        Assert.Equal(100 + expected, queue.take(100, _ => true)!.Deadline);
    }

    [Fact]
    public void usersStayInArrivalOrderAheadOfSystemsWithoutInterruptingActiveWork()
    {
        var queue = new ActionQueue();
        _ = queue.enqueue(action("system1", "system1") with { Author = "system" }, 0, 10000, 999, TestContext.Current.CancellationToken);
        var active = queue.take(0, _ => true)!;
        _ = queue.enqueue(action("system2", "system2", "high") with { Author = "system" }, 1, 10000, 999, TestContext.Current.CancellationToken);
        _ = queue.enqueue(action("user1", "user1", "low"), 2, 30000, 999, TestContext.Current.CancellationToken);
        _ = queue.enqueue(action("user2", "user2", "high"), 3, 30000, 999, TestContext.Current.CancellationToken);
        _ = queue.enqueue(action("system3", "system3") with { Author = "system" }, 4, 10000, 999, TestContext.Current.CancellationToken);
        Assert.Equal("system1", queue.currentAction!.Id);
        Assert.Null(queue.take(5, _ => true));
        queue.pause(active, "KEYBOARD_ACTIVE");
        foreach (var id in new[] { "user1", "user2", "system1", "system2", "system3" })
        {
            Assert.Equal(id, queue.currentAction!.Id);
            var next = queue.take(6, _ => true)!;
            Assert.Equal(id, next.Request.Id);
            queue.finish(next, "completed");
        }
    }

    [Fact]
    public void blockedUserDoesNotPreventEligibleSystemWork()
    {
        var queue = new ActionQueue();
        _ = queue.enqueue(action("system", "system") with { Author = "system" }, 0, 10000, 999, TestContext.Current.CancellationToken);
        _ = queue.enqueue(action("user", "user"), 1, 30000, 999, TestContext.Current.CancellationToken);
        Assert.Equal("system", queue.take(2, request => request.Author == "system")!.Request.Id);
    }
