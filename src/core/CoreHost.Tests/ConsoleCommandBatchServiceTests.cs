...
public sealed class ConsoleCommandBatchServiceTests
{
    ...
    [Fact]
    public async Task LastCommandOnlyTracksSubmittedBatchCommands()
    {
        var activity = new ConsoleCommandActivity();
        Assert.Null(activity.GetSnapshot());
        var runtime = new FakeConsoleBatchRuntime
        {
            SubmitResults = [Submitted(), Failed("INPUT_FAILED", "Not submitted")]
        };
        var service = CreateService(runtime, commandActivity: activity);

        await service.ExecuteAsync("debug", Commands("ListPlayers", "unsent"), null, CancellationToken.None);

        Assert.Equal("ListPlayers", activity.GetSnapshot()!.Command);
    }
    ...
    private static ConsoleCommandBatchService CreateService(
        ...
        ConsoleCommandActivity? commandActivity = null)
    {
        ...
        return new ConsoleCommandBatchService(
            ...
            commandActivity ?? new ConsoleCommandActivity());
    }
