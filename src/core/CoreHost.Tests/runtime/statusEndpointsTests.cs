...
public sealed class StatusEndpointsTests
{
    ...
    public async Task runtimeGameStateSelectsKeyboardUsedMouseVisibleCooldown(bool isMainMenu, int cooldownMs)
    {
        await using var host = await RuntimeEndpointHost.start();
        using var state = await host.send("POST", "/v3/runtime/game", new { isMainMenu });
        Assert.Equal(HttpStatusCode.OK, state.StatusCode);
        host.native.foreground = 2;
        host.native.cursorFlags = 1;
        ...
        Assert.InRange(json.RootElement.GetProperty("data").GetProperty("input")
            .GetProperty("keyboardUsedMouseVisibleCooldownRemainingMs").GetInt64(), cooldownMs - 1000, cooldownMs);
    }

    [Fact]
    public async Task keyboardWithVisibleCursorBlocksCommandsAndKeepsBlockingAfterItHides()
    {
        await using var host = await RuntimeEndpointHost.start();
        ...
        input.key(65, true, false);
        input.key(65, false, false);
        foreach (var flags in new uint[] { 1, 0 })
        {
            host.native.cursorFlags = flags;
            using var response = await host.send("GET", "/v3/status");
            using var json = await RuntimeEndpointHost.json(response);
            var execution = json.RootElement.GetProperty("data").GetProperty("execution");
            Assert.False(execution.GetProperty("canExecuteCommand").GetBoolean());
            Assert.Equal("KeyboardUsedMouseVisible", execution.GetProperty("reason").GetString());
        }
    }
    ...
    [InlineData(true, 1)]
    [InlineData(false, 2)]
    public async Task statusAndMetaExposeMouseShowing(bool running, int foreground)
    {
        await using var host = await RuntimeEndpointHost.start(running);
        host.native.foreground = foreground;
        foreach (var flags in new uint?[] { 0, 1, 2, 3, null })
        foreach (var path in new[] { "/v3/status", "/v2/meta/get" })
        {
            host.native.cursorFlags = flags;
            using var response = await host.send("GET", path);
            using var json = await RuntimeEndpointHost.json(response);
            Assert.Equal(running && foreground == 2 && flags is 1 or 3,
                json.RootElement.GetProperty("data").GetProperty("mouseShowing").GetBoolean());
        }
    }

    [Fact]
    public async Task mouseShowingIsFalseWhenFocusChangesDuringCursorRead()
    {
        await using var host = await RuntimeEndpointHost.start();
        host.native.cursorFlags = 1;
        host.native.onCursorRead = () => host.native.foreground = 1;
        foreach (var path in new[] { "/v3/status", "/v2/meta/get" })
        {
            host.native.foreground = 2;
            using var response = await host.send("GET", path);
            using var json = await RuntimeEndpointHost.json(response);
            Assert.False(json.RootElement.GetProperty("data").GetProperty("mouseShowing").GetBoolean());
        }
    }

    [Fact]
    public async Task roundTripReportsConsistentOrdinaryEffectiveAndLegacyActivity()
    {
        await using var host = await RuntimeEndpointHost.start();
        using var registration = await host.send("POST", "/v3/runtime/app", new AppTarget(11, "0x1"));
        var runtime = host.services.GetRequiredService<GameActionRuntime>();
        var tracker = host.services.GetRequiredService<InputActivityTracker>();
        host.native.foreground = 1;
        var action = new QueuedAction(RuntimeEndpointHost.action(new CoreCommand("console", "Serversay test", "NumpadSubtract")),
            Environment.TickCount64 + 10000, TestContext.Current.CancellationToken);
        Assert.True((await runtime.prepare(action, TestContext.Current.CancellationToken)).Ok);
        tracker.key(0x59, true, false);
        tracker.mouse(1, true, false);

        using var response = await host.send("GET", "/v3/debug");
        using var json = await RuntimeEndpointHost.json(response);
        var data = json.RootElement.GetProperty("data");
        Assert.True(data.GetProperty("ordinaryGate").GetProperty("canExecuteCommand").GetBoolean());
        var status = data.GetProperty("status");
        Assert.True(status.GetProperty("execution").GetProperty("canExecuteCommand").GetBoolean());
        foreach (var field in new[] { "keyboardActive", "mouseActive", "isChatting" })
            Assert.False(status.GetProperty("input").GetProperty(field).GetBoolean());
        Assert.True(status.GetProperty("input").GetProperty("checkLockActive").GetBoolean());
        using var legacy = await host.send("GET", "/v2/meta/get");
        using var legacyJson = await RuntimeEndpointHost.json(legacy);
        var movement = legacyJson.RootElement.GetProperty("data").GetProperty("movement");
        Assert.False(movement.GetProperty("isMoving").GetBoolean());
        Assert.False(movement.GetProperty("isChatting").GetBoolean());
        Assert.True(movement.GetProperty("checkLockActive").GetBoolean());
        await runtime.cleanup(TestContext.Current.CancellationToken);
        Assert.True(runtime.canExecuteCommand().Allowed);
        host.native.foreground = 2;
        Assert.Equal("KEYBOARD_ACTIVE", runtime.canExecuteCommand().Reason);
    }
    ...
    [InlineData(false, 2)]
    [InlineData(false, 1)]
    public async Task gameInputStatusIsInactiveOutsideTheGameAndAppFocusSuppressesActivityGates(bool running, int foreground)
    {
        await using var host = await RuntimeEndpointHost.start(running);
        using var registration = await host.send("POST", "/v3/runtime/app", new AppTarget(11, "0x1"));
        host.native.foreground = foreground;
        ...
        var appFocused = foreground == 1;
        Assert.Equal(running && appFocused, data.GetProperty("execution").GetProperty("canExecuteCommand").GetBoolean());
        Assert.Equal(appFocused, input.GetProperty("checkLockActive").GetBoolean());
        Assert.Equal(!appFocused, tracker.snapshot(0).KeyboardActive);
        Assert.Equal(!appFocused, tracker.snapshot(0).MouseActive);
        using var legacy = await host.send("GET", "/v2/meta/get");
        using var legacyJson = await RuntimeEndpointHost.json(legacy);
