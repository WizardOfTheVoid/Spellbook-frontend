...
public sealed class CommandTextBuilderTests
{
    ...
    [Fact]
    public void BuildMessagePreservesPreparedTextExactly()
    {
        var result = CreateBuilder().BuildMessage(
            new ConsoleMessageRequest("message-1", "admin", "  M∆GIC  ♥\t你好  ", null));

        Assert.True(result.Ok);
        Assert.Equal("Adminsay \"  M∆GIC  ♥\t你好  \"", result.Command);
    }
    ...
    [Fact]
    public void BuildMessageRejectsEmbeddedDoubleQuotes()
    {
    ...
    [Theory]
    [InlineData(null, "Message is required.")]
    [InlineData("", "Message is required.")]
    [InlineData(" \t ", "Message is required.")]
    [InlineData("\n", "Message cannot contain CR, LF, or NUL.")]
    [InlineData("line\rbreak", "Message cannot contain CR, LF, or NUL.")]
    [InlineData("line\0break", "Message cannot contain CR, LF, or NUL.")]
    ...
    public void BuildMessageEnforcesLengthWithoutTruncation(int length, bool expectedOk)
    {
        var message = new string('x', length);

        var result = CreateBuilder().BuildMessage(
            new ConsoleMessageRequest("message-1", "server", message, null));

        Assert.Equal(expectedOk, result.Ok);
        Assert.Equal(expectedOk ? $"Serversay \"{message}\"" : null, result.Command);
        Assert.Equal(expectedOk ? null : "INVALID_REQUEST", result.ErrorCode);
    }
    ...
    [Fact]
    public void BuildBatchPreservesEveryPreparedMessageAndReasonExactly()
    {
        var builder = CreateBuilder();
        var result = builder.BuildBatch(new ConsoleBatchRequest(
            "batch-1",
            [
                new ConsoleBatchCommandRequest("server_message", null, null, " M∆GIC  ♥ 你好 ", 0),
                new ConsoleBatchCommandRequest("warn", null, null, " M∆GIC  ♥ 你好 ", 0),
                new ConsoleBatchCommandRequest("kick", "PLAYER_1", null, " M∆GIC  ♥ 你好 ", 0),
                new ConsoleBatchCommandRequest("ban", "PLAYER_2", 24, " M∆GIC  ♥ 你好 ", 0)
            ],
            null));
        ...
        Assert.Equal(
            [
                "Serversay \" M∆GIC  ♥ 你好 \"",
                "Serversay \" M∆GIC  ♥ 你好 \"",
                "KickById PLAYER_1 \" M∆GIC  ♥ 你好 \"",
                "BanById PLAYER_2 24 \" M∆GIC  ♥ 你好 \""
            ],
            result.Commands!.Select(command => command.Command));
    }
    ...
    public static IEnumerable<object[]> InvalidBatchRequests()
    {
        ...
        foreach (var commandType in new[] { "server_message", "warn", "kick", "ban" })
        {
            foreach (var message in new string?[] { null, "", " \t ", "Say \"hello\"", "line\nbreak", "line\rbreak", "line\0break", new('x', 181) })
            {
                yield return [$"invalid {commandType} text", Batch([Warn(), new ConsoleBatchCommandRequest(commandType, "PLAYER_1", 24, message, 0)])];
            }
        }
    }

    [Fact]
    public void RawCommandValidationPreservesSymbolsAndQuotes()
    {
