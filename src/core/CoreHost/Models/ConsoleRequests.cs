...
public static class RequestValidators
{
    ...
    public static bool TryValidateQuotedText(string? text, string fieldName, out string? errorMessage)
    {
        if (text is not null && ContainsForbiddenLineCharacter(text))
        {
        ...
        if (text.Contains('"'))
        {
            errorMessage = $"{fieldName} cannot contain double quotes.";
            return false;
        }

        if (text.Length > MaxQuotedTextLength)
        {
            errorMessage = $"{fieldName} must be {MaxQuotedTextLength} characters or fewer.";
            return false;
        }
