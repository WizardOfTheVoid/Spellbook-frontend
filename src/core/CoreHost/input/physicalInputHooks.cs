...
internal sealed class PhysicalInputHooks : IPhysicalInputHooks
{
    ...
    private IntPtr mouse(int code, IntPtr message, IntPtr pointer)
    {
        if (code >= 0)
        {
            ...
            var down = eventCode is NativeMethods.WM_LBUTTONDOWN or NativeMethods.WM_RBUTTONDOWN or NativeMethods.WM_MBUTTONDOWN or NativeMethods.WM_XBUTTONDOWN;
            var injected = (data.Flags & NativeMethods.LLMHF_INJECTED) != 0;
            if (button != 0 || injected) activity.mouse(button, down, injected);
            if (activity.suppressMouseClick(button, down, injected)) return new IntPtr(1);
        }
