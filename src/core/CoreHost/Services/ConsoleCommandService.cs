...
public sealed class ConsoleCommandService
{
    ...
    private readonly ConsoleCommandActivity _commandActivity;
        ...
        Action<Func<Task<ConsoleExecutionResult>>> deferCleanup)
    {
        ...
        try
        {
            ...
            OperationResult enter;
            try
            {
                enter = await timing.MeasureAsync(
                    "enter",
            ...
            {
                if (consoleOpenAttempt.CommandSubmitted) _commandActivity.Record(command);
            }
            if (!enter.Ok)
            {
