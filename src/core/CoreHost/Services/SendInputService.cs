...
public sealed class SendInputService(IKeyboardInputApi inputApi, ConsoleKeySelection? consoleKey = null) : IConsoleCleanupInput
{
    ...
    {
        if (consoleKey?.Code is { } code)
        {
            if (!ConsoleKeySelection.TryGetKey(code, out var key) || key is null)
            {
                return OperationResult.Failure("INVALID_REQUEST", "Console key must be a supported physical key code.");
            }

            return await SendScanCodeAsync(key.ScanCode, timing.BetweenKeyDelayMs, cancellationToken, attemptState, key.Extended).ConfigureAwait(false);
        }
    ...
    private async Task<OperationResult> SendScanCodeAsync(
        ...
        bool extended = false)
    {
        var result = SendInputs(CreateScanCodeInputs(scanCode, extended), attemptState);
        if (!result.Ok)
        {
            return result;
        }
    ...
    private static KeyboardInputEvent[] CreateScanCodeInputs(ushort scanCode, bool extended)
    {
        var flags = NativeMethods.KEYEVENTF_SCANCODE | (extended ? 0x0001u : 0u);
        return
        [
            new KeyboardInputEvent(0, scanCode, flags),
            new KeyboardInputEvent(0, scanCode, flags | NativeMethods.KEYEVENTF_KEYUP)
        ];
    }
}
