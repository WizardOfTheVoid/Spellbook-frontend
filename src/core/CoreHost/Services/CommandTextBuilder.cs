...
public sealed class CommandTextBuilder
{
    ...
    public CommandBuildResult BuildBan(BanRequest request)
    {
        ...
        if (!RequestValidators.TryValidateQuotedText(request.Reason, "Reason", out var reasonError))
        {
            return CommandBuildResult.Failure("INVALID_REQUEST", reasonError!, requestId);
        }

        return CommandBuildResult.Success(requestId, $"BanById {playfabId} {request.Hours.Value} \"{request.Reason}\"");
    }
    ...
    public CommandBuildResult BuildKick(KickRequest request)
    {
        ...
        return CommandBuildResult.Success(requestId, $"KickById {playfabId} \"{request.Reason}\"");
    }
    ...
    public CommandBuildResult BuildWarn(WarnRequest request)
    {
        ...
        if (!RequestValidators.TryValidateQuotedText(request.Message, "Message", out var messageError))
        {
            return CommandBuildResult.Failure("INVALID_REQUEST", messageError!, requestId);
        }

        return CommandBuildResult.Success(requestId, $"Serversay \"{request.Message}\"");
    }
    ...
    public CommandBuildResult BuildMessage(ConsoleMessageRequest request)
    {
        ...
        return CommandBuildResult.Success(requestId, $"{command} \"{request.Message}\"");
    }
