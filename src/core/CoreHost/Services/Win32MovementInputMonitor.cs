...
{
    private readonly Channel<MovementKeyDown> _movements = Channel.CreateBounded<MovementKeyDown>(
    ...
    {
        if (code >= 0 &&
            (message == new IntPtr(NativeMethods.WM_KEYDOWN) ||
                message == new IntPtr(NativeMethods.WM_SYSKEYDOWN)))
        {
            var data = Marshal.PtrToStructure<NativeMethods.KBDLLHOOKSTRUCT>(dataPointer);
            var isInjected = (data.Flags & NativeMethods.LLKHF_INJECTED) != 0;
            if (!isInjected && MovementActivityTracker.IsMovementKey((int)data.VirtualKey))
            {
                _movements.Writer.TryWrite(new MovementKeyDown(
                    (int)data.VirtualKey,
    ...
    {
        await foreach (var movement in _movements.Reader.ReadAllAsync().ConfigureAwait(false))
        {
            try
            {
    ...
    private readonly record struct MovementKeyDown(
        int VirtualKey,
